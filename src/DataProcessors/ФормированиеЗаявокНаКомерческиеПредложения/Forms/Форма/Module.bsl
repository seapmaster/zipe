...
&НаКлиенте
Функция ПолучитьКаталогДляСохраненияНабораФайлов()
...
&НаКлиенте
Процедура СоздатьПодкаталог(Каталог, Подкаталог)
	
	КаталогНаДиске = Новый Файл(Каталог + Подкаталог);
	Если НЕ КаталогНаДиске.Существует() Тогда
		
		СоздатьКаталог(Каталог + Подкаталог);
...
Процедура СохранитьФайлыВКаталог(МассивДокументов, Каталог, Результат)
	...
	Попытка
		...
		Для Каждого Документ Из МассивДокументов Цикл
			...
			Если Документ.Типфайла = "ТабличныйДокумент" Тогда
				СохраняемыйДокумент = Документ.Данные;
				СохраняемыйДокумент.Записать(КаталогВыгрузки + Документ.ИмяФайла, Документ.ТипФайлаДокумента);
			Иначе
				СохраняемыйДокумент = Документ.Данные;
				ЗаписатьWord(КаталогВыгрузки + Документ.ИмяФайла, СохраняемыйДокумент);				
			КонецЕсли;
...
&НаКлиенте
Процедура ЗаписатьWord(ПутьКФайлу, ТекстПисьма)
	
	Попытка
		
		WordПриложение = Новый COMОбъект("Word.Application");
		WordДокумент = WordПриложение.Documents.Add();
	...
	WordДокумент.Application.Selection.TypeText(ТекстПисьма);
	
	Попытка
		
		WordДокумент.SaveAs(ПутьКФайлу);
		WordДокумент.Application.Quit();
...
&НаКлиенте
Процедура СформироватьНаборФайлов(Команда)
	
	Каталог = ПолучитьКаталогДляСохраненияНабораФайлов();
