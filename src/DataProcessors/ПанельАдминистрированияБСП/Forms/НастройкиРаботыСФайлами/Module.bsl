...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ФайловыеФункции") Тогда
		ВызватьИсключение НСтр("ru = 'Не внедрена подсистема Файловые функции.
	...
	МаксимальныйРазмерФайла              = МодульФайловыеФункции.МаксимальныйРазмерФайлаОбщий() / (1024*1024);
	МаксимальныйРазмерФайлаОбластиДанных = МодульФайловыеФункции.МаксимальныйРазмерФайла() / (1024*1024);
...
Процедура НастройкаОчисткиФайлов(Команда)
	ИмяОткрываемойФормы = "РегистрСведений.НастройкиОчисткиФайлов.ФормаСписка";
	ОткрытьФорму(ИмяОткрываемойФормы, , ЭтотОбъект);
КонецПроцедуры
...
&НаКлиенте
Процедура ВыгружатьФайлыВоВнешнееХранилищеПриИзменении(Элемент)
	УстановитьВыгрузкуФайловВоВнешнееХранилище(НаборКонстант.ВыгружатьФайлыВоВнешнееХранилище);	
КонецПроцедуры // ВыгружатьФайлыВоВнешнееХранилищеПриИзменении

&НаСервереБезКонтекста
Процедура УстановитьВыгрузкуФайловВоВнешнееХранилище(ЗначениеКонстанты)
	Константы.ВыгружатьФайлыВоВнешнееХранилище.Установить(ЗначениеКонстанты);
КонецПроцедуры // УстановитьВыгрузкуФайловВоВнешнееХранилище
