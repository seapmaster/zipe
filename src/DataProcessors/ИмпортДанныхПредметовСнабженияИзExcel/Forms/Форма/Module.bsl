...
Процедура ПолучитьКоличествоСтрокВФайлах(МассивФайлов)
	...
	Для каждого Файл Из МассивФайлов Цикл
		...
		ПараметрыФайла = РаботаСФайламиOfficeКлиент.ПодготовитьФайлКЧтению(Файл.ПолноеИмя, "ТабличныйДокумент");
		...
		Для Каждого ОписаниеЛиста Из ЛистыДокумента Цикл
			...
			Если ЗначениеЗаполнено(СокрЛП(Строка(Лист.Область(1,1).Текст))) Тогда
				ОбщееКоличествоСтрокФайлов = ОбщееКоличествоСтрокФайлов + Лист.ВысотаТаблицы;
				Прервать;
			КонецЕсли;
...
Процедура ПрочитатьДанныеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Иначе
			...
			НачатьПоискФайлов(Новый ОписаниеОповещения("ПрочитатьДанныеЗавершениеЗавершение", ЭтотОбъект, Новый Структура("СоответствиеТиповУзловСтруктурыКорабля", СоответствиеТиповУзловСтруктурыКорабля)), ПутьКФайлам, "*.xls", Истина);
            Возврат;
			
		КонецЕсли;
		
		ПрочитатьДанныеЗавершениеФрагмент(МассивФайлов, СоответствиеТиповУзловСтруктурыКорабля);
...
&НаКлиенте
Процедура ПрочитатьДанныеЗавершениеЗавершение(НайденныеФайлы, ДополнительныеПараметры1) Экспорт
	
	СоответствиеТиповУзловСтруктурыКорабля = ДополнительныеПараметры1.СоответствиеТиповУзловСтруктурыКорабля;
...
&НаКлиенте
Процедура ПрочитатьДанныеЗавершениеФрагмент(Знач МассивФайлов, Знач СоответствиеТиповУзловСтруктурыКорабля)
	
	Перем ДатаНачала, КоличествоФайлов, ПервыйСимволИмени, Сч, Файл;
...
Процедура ПрочитатьФайл(Файл, СоответствиеТиповУзловСтруктурыКорабля)
	...
	Если ИспользоватьЗагрузчик1с Тогда //читаем через Excel.Application
...
Процедура ПрочитатьДанныеПроект877(Источник, СоответствиеТиповУзловСтруктурыКорабля)
	...
	Если ИспользоватьЗагрузчик1с Тогда
		
		ВсегоСтрок = Источник.ВысотаТаблицы;
	...
	Пока ?(ИспользоватьЗагрузчик1с, Сч <= ВсегоСтрок, Источник.EOF()=0) Цикл
		
		Если ИспользоватьЗагрузчик1с Тогда //Excel.Application читает файлы медленно. отображаем процесс чтения
		...
		Если Не ЗначениеЗаполнено(Уровень) И ТипУзла = Неопределено Тогда //это пустая строка. пропукаем
			...
			Если Не ИспользоватьЗагрузчик1с Тогда
...
Процедура ПрочитатьДанныеАлмаз(Источник, СоответствиеТиповУзловСтруктурыКорабля)
	...
	ИндексПервойСтроки = 4 + ?(ИспользоватьЗагрузчик1с, 1, 0);
...
Функция ПолучитьЧисловоеЗначение(Источник, ИндексКолонки)
	...
	Попытка
		...
		ЗначениеПоля = ?(ИспользоватьЗагрузчик1с, Источник.Область(НомерТекущейСтроки, ИндексКолонки + 1).Текст, Источник.Fields(ИндексКолонки).Value);
		...
		Если ЗначениеПоля = Null И ИспользоватьЗагрузчик1с Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ВыводитьОбщееКоличествоЗаписей.Видимость = ИспользоватьЗагрузчик1с;
