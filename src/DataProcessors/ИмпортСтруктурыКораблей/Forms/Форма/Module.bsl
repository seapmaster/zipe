...
Процедура ЗагрузитьПриложение1()
	...
	ПараметрыФайла = РаботаСФайламиOfficeКлиент.ПодготовитьФайлКЧтению(ИмяФайла, "ТабличныйДокумент");
	Если ПараметрыФайла = Неопределено Тогда
		Возврат;
	КонецЕсли; // Если ПараметрыФайла = Неопределено Тогда
	
	ЛистЭксель = РаботаСФайламиOffice.ПрочитатьФайлВТабличныйДокумент(ПараметрыФайла.Адрес, ПараметрыФайла);
	Если ЛистЭксель = Неопределено Тогда
		Возврат;
	КонецЕсли; // Если ЛистЭксель = Неопределено Тогда
	...
	Для сч = 5 По ВсегоСтрок Цикл // Самую первую пропускаем
		...
		Стр1 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 1).Текст));
		Стр2 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 4).Текст));
		Стр3 = СокрЛП(УдалитьЛишниеСимволыБезПереноса(ЛистЭксель.Область(сч, 5).Текст));
		Стр4 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 6).Текст));
		Стр5 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 7).Текст));
		Стр6 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 8).Текст));
		Стр7 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 9).Текст));
		Стр8 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 11).Текст));
		Стр9 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 13).Текст));
		
		стр11 = СокрЛП(ЛистЭксель.Область(сч, 2).Текст);
		стр12 = СокрЛП(ЛистЭксель.Область(сч, 3).Текст);
...
Процедура ЗагрузитьПриложение2()
	...
	Если ЛистыДокумента.Количество() < 2 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Неверная структура файла");
	КонецЕсли; // Если ЛистыДокумента.Количество() < 2 Тогда
	
	Табл877.Очистить();
	
	
	// Лист оборудование
	ЛистЭксель = ЛистыДокумента.Лист_1;
	...
	Для сч = 3 По ВсегоСтрок Цикл // Самую первую пропускаем
		...
		Стр1 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 14).Текст));
		Стр2 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 1).Текст));
		Стр3 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 2).Текст));
		Стр31 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 3).Текст));
		Стр4 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 7).Текст));
		Стр5 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 5).Текст));
		Стр51 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 6).Текст));
		Стр6 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 8).Текст));
		Стр7 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 9).Текст));
	...
	// Лист материалы
	ЛистЭксель = ЛистыДокумента.Лист_2;
	...
	Для сч = 3 По ВсегоСтрок Цикл // Самую первую пропускаем
		...
		Стр1 = СокрЛП(ЛистЭксель.Область(сч, 7).Текст);
		Стр2 = СокрЛП(ЛистЭксель.Область(сч, 1).Текст);
		Стр3 = ""; //СокрЛП(ЛистЭксель.Область(сч, 2).Текст);
		Стр4 = СокрЛП(ЛистЭксель.Область(сч, 4).Текст);
		Стр5 = СокрЛП(ЛистЭксель.Область(сч, 3).Текст)+" "+СокрЛП(ЛистЭксель.Область(сч, 5).Текст);
		Стр6 = СокрЛП(ЛистЭксель.Область(сч, 8).Текст);
		Стр7 = СокрЛП(ЛистЭксель.Область(сч, 9).Текст);
		Стр8 = ""; //СокрЛП(ЛистЭксель.Область(сч, 9).Текст);
		Стр9 = ""; //СокрЛП(ЛистЭксель.Область(сч, 11).Текст);
...
Процедура ЗагрузитьПриложение3()
	...
	Для сч = 3 По ЛистЭксель.ВысотаТаблицы Цикл // Самую первую пропускаем
		
		Стр1 	= СокрЛП(ЛистЭксель.Область(сч, 3).Текст);
		Стр2	= ""; //СокрЛП(ЛистЭксель.Область(сч, 2).Текст);
		Стр3 	= СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 6).Текст));
		Стр31 	= СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 7).Текст));
		Стр4 	= СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 8).Текст));
		Стр5 	= СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 9).Текст));
		Стр6 	= СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 13).Текст));
		Стр7 	= СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 12).Текст));
		Стр8 	= ""; //СокрЛП(ЛистЭксель.Область(сч, 9).Текст);
		Стр9 	= ""; //СокрЛП(ЛистЭксель.Область(сч, 11).Текст);
...
Процедура ЗагрузитьЦены()
	...
	Для сч = 5 По ЛистЭксель.ВысотаТаблицы Цикл // Самую первую пропускаем
		
		Стр1 = СокрЛП(ЛистЭксель.Область(сч, 1).Текст);
		Стр2 = СокрЛП(УдалитьЛишниеСимволыБезПереноса(ЛистЭксель.Область(сч, 2).Текст));
		Стр3 = СокрЛП(УдалитьЛишниеСимволыБезПереноса(ЛистЭксель.Область(сч, 3).Текст));
		Стр4 = ""; //СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 4).Текст));
		Стр5 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 5).Текст));
		Стр6 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 6).Текст));
		Стр7 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 7).Текст));
...
Процедура Заполнить956_ДЭ()
	НачатьПоискФайлов(Новый ОписаниеОповещения("Заполнить956_ДЭЗавершение", ЭтотОбъект), ИмяФайла, "*ДЭ*.doc", Истина); 
КонецПроцедуры

&НаКлиенте
Процедура Заполнить956_ДЭЗавершение(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	
	НайдФайлы = НайденныеФайлы;
...
Процедура Заполнить1234_Эксель()
	...
	Для сч = 12 По ЛистЭксель.ВысотаТаблицы Цикл // Самую первую пропускаем
		
		Стр1 = СокрЛП(ЛистЭксель.Область(сч, 2).Текст);
		Стр2 = СокрЛП(ЛистЭксель.Область(сч, 3).Текст);
		Стр3 = СокрЛП(ЛистЭксель.Область(сч, 4).Текст);
		Стр4 = СокрЛП(ЛистЭксель.Область(сч, 5).Текст);
		Стр5 = ""; //СокрЛП(ЛистЭксель.Область(сч, 7).Текст);
		Стр6 = СокрЛП(ЛистЭксель.Область(сч, 6).Текст);
		Стр7 = ""; //СокрЛП(ЛистЭксель.Область(сч, 9).Текст);
		Стр8 = ""; //СокрЛП(ЛистЭксель.Область(сч, 10).Текст);
		...
		НовСтр = Табл956.Добавить();
		НовСтр.Позиция 		 = Стр1;
		НовСтр.Наименование  = Стр2 + Символы.ПС + СокрЛП(ЛистЭксель.Область(сч+1, 3).Текст);;   
		НовСтр.Обозначение   = Стр3;
		НовСтр.ЕдИзмерения   = Стр4;
		НовСтр.МассаЕдиницы  = Стр5;
		НовСтр.Количество 	 = Стр6;
...
Процедура Заполнить1234_Эксель_2()
	...
	Для сч = 3 По ЛистЭксель.ВысотаТаблицы Цикл // Самую первую пропускаем
		...
		Стр1 = СокрЛП(ЛистЭксель.Область(сч,  9).Текст);
		Стр2 = СокрЛП(ЛистЭксель.Область(сч, 10).Текст);
		Стр3 = ""; //СокрЛП(ЛистЭксель.Область(сч, 4).Текст);
		Стр4 = ""; //СокрЛП(ЛистЭксель.Область(сч, 5).Текст);
		Стр5 = ""; //СокрЛП(ЛистЭксель.Область(сч, 7).Текст);
		Стр6 = ""; //СокрЛП(ЛистЭксель.Область(сч, 6).Текст);
...
Процедура Заполнить1159_Эксель()
	...
	Для сч = 14 По ЛистЭксель.ВысотаТаблицы Цикл // Самую первую пропускаем
...
Процедура Заполнить1159_Эксель_2()
	...
	Для сч = 3 По ЛистЭксель.ВысотаТаблицы Цикл // Самую первую пропускаем
		...
		Стр1 = СокрЛП(ЛистЭксель.Область(сч, 8).Текст);
		Стр2 = СокрЛП(ЛистЭксель.Область(сч, 9).Текст);
		Стр3 = ""; //СокрЛП(ЛистЭксель.Область(сч, 4).Текст);
		Стр4 = ""; //СокрЛП(ЛистЭксель.Область(сч, 5).Текст);
		Стр5 = ""; //СокрЛП(ЛистЭксель.Область(сч, 7).Текст);
		Стр6 = ""; //СокрЛП(ЛистЭксель.Область(сч, 6).Текст);
...
Процедура Заполнить11661()
	...
	Каталог = ИмяФайла;
	НачатьПоискФайлов(Новый ОписаниеОповещения("Заполнить11661Завершение", ЭтотОбъект), Каталог, "*.doc", Истина); 
	
КонецПроцедуры

&НаКлиенте
Процедура Заполнить11661Завершение(НайденныеФайлы, ДополнительныеПараметры) Экспорт
...
Процедура Заполнить11356()
	Каталог = ИмяФайла;
	НачатьПоискФайлов(Новый ОписаниеОповещения("Заполнить11356Завершение", ЭтотОбъект), Каталог, "*.doc", Истина); 
КонецПроцедуры

&НаКлиенте
Процедура Заполнить11356Завершение(НайденныеФайлы, ДополнительныеПараметры) Экспорт
...
Процедура Загрузить06360_1()
	...
	Для сч = 5 По ЛистЭксель.ВысотаТаблицы Цикл // Самую первую пропускаем
		...
		Стр1  = ""; //СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 14).Текст));
		Стр2  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 4).Текст));
		Стр3  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 10).Текст));
		Стр31 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 12).Текст));
		Стр4  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 16).Текст));
		Стр5  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 25).Текст));
		Стр6  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 30).Текст));
		Стр7  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 17).Текст));
		Стр8  = СокрЛП(ЛистЭксель.Область(сч, 14).Текст);
		Стр9  = СокрЛП(ЛистЭксель.Область(сч, 15).Текст);
		Стр10 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 23).Текст));
		Стр11 = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 18).Текст));
...
Процедура Загрузить06360_2()
	...
	Если Не ЛистыДокумента.Свойство("Лист_2") Тогда
		Возврат;
	КонецЕсли; // Если Не ЛистыДокумента.Свойство("Лист_2") Тогда
	...
	Для сч = 2 По ВсегоСтрок Цикл // Самую первую пропускаем
		...
		Стр1  = ""; //СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 14).Текст));
		Стр2  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 4).Текст));
		Стр3  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 14).Текст));
		Стр31 = ""; //СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 12).Текст));
		Стр4  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 11).Текст));
		Стр5  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 33).Текст));
		Стр6  = ""; //СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 30).Текст));
		Стр7  = СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 12).Текст));
		Стр8  = ""; //СокрЛП(ЛистЭксель.Область(сч, 14).Текст);
		Стр9  = СокрЛП(ЛистЭксель.Область(сч, 13).Текст);
		Стр10 = ""; //СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 23).Текст));
		Стр11 = ""; //СокрЛП(УдалитьЛишниеСимволы(ЛистЭксель.Область(сч, 18).Текст));
...
Функция ПолучитьЗначениеВЯчейке(ЛистЭксель,НомерСтроки,НомерСтолбца)
	...
	ЗначениеЯч = ЛистЭксель.Область(НомерСтроки,НомерСтолбца).Value;
...
Процедура ЗаполнитьАврора()
	...
	Иначе
		НачатьПоискФайлов(Новый ОписаниеОповещения("ЗаполнитьАврораЗавершение", ЭтотОбъект, Новый Структура("ДатаНачалаЗагрузки, СоответствиеВидовХарактеристик, СоответствиеРасположений", ДатаНачалаЗагрузки, СоответствиеВидовХарактеристик, СоответствиеРасположений)), ТекПутьКФайлу+"\"+ТекИмяФайла,"*.xls",Истина);
        Возврат;
	КонецЕсли;
	ЗаполнитьАврораФрагмент(ДатаНачалаЗагрузки, СоответствиеВидовХарактеристик, СоответствиеРасположений, ФайлыЗИП);
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьАврораЗавершение(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	
	ДатаНачалаЗагрузки 				= ДополнительныеПараметры.ДатаНачалаЗагрузки;
	СоответствиеВидовХарактеристик 	= ДополнительныеПараметры.СоответствиеВидовХарактеристик;
	СоответствиеРасположений 		= ДополнительныеПараметры.СоответствиеРасположений;
	ФайлыЗИП						= НайденныеФайлы;
	ЗаполнитьАврораФрагмент(ДатаНачалаЗагрузки, СоответствиеВидовХарактеристик, СоответствиеРасположений, ФайлыЗИП);

КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьАврораФрагмент(ДатаНачалаЗагрузки, Знач СоответствиеВидовХарактеристик, Знач СоответствиеРасположений, Знач ФайлыЗИП)
	
	Перем ВсегоСтрок, ВсегоЭлементовФайлыЗИП, ЗначениеПервойЯчейки, КаталогЗаказа, КаталогИзделия, КаталогПроекта, ЛистЭксель, ЛыстыДокумента, НаименованиеЗаказа, НаименованиеЗаказчика, НаименованиеИзделия, НаименованиеПроекта, ОписаниеЛиста, ПараметрыФайла, ПерваяСтрока, РасположениеЗИП, СтрАврора, Сч, ТекНаименование, ТекНомерЭлементаФайлыЗИП, ТекОписание, ФайлЗИП;
	...
	Для каждого ФайлЗИП Из ФайлыЗИП Цикл
		...
		ЛыстыДокумента = РаботаСФайламиOffice.ПрочитатьВсеЛистыФайла(ПараметрыФайла);
		Если ЛыстыДокумента = Неопределено Тогда
			ТекНомерЭлементаФайлыЗИП=ТекНомерЭлементаФайлыЗИП+1;
			Продолжить;
		КонецЕсли; // Если ЛыстыДокумента = Неопределено Тогда
		
		Для Каждого ОписаниеЛиста Из ЛыстыДокумента Цикл
			ЛистЭксель = ОписаниеЛиста.Значение;
			Если ЗначениеЗаполнено(СокрЛП(Строка(ЛистЭксель.Область(1,1).Текст))) Тогда
				Прервать;
			КонецЕсли; 
		КонецЦикла; // Для Каждого ОписаниеЛиста Из ЛыстыДокумента Цикл
...
&НаКлиенте
Асинх Процедура ЗаполнитьГепард()
	...
	Если ТаблГепард.Количество() = 0 Или ЗагружатьВЗаполненныеТаблицы Тогда 
		...
		Иначе
			МассФайлов = Ждать НайтиФайлыАсинх(ИмяФайла, "*.xls",Истина);
		КонецЕсли;
...
Процедура Загрузить1241РЭ_1()
	...
	Для сч = 5 По ВсегоСтрок Цикл // Самую первую пропускаем
		...
		Стр8  = СокрЛП(ЛистЭксель.Область(сч, 8).Текст);
		Стр9  = СокрЛП(ЛистЭксель.Область(сч, 9).Текст);
		Стр10 = СокрЛП(ЛистЭксель.Область(сч, 10).Текст);
		Стр11 = СокрЛП(ЛистЭксель.Область(сч, 11).Текст);
		Стр12 = СокрЛП(ЛистЭксель.Область(сч, 12).Текст);
		Стр13 = СокрЛП(ЛистЭксель.Область(сч, 13).Текст);
		Стр14 = СокрЛП(ЛистЭксель.Область(сч, 14).Текст);
		Стр15 = СокрЛП(ЛистЭксель.Область(сч, 15).Текст);
		Стр16 = СокрЛП(ЛистЭксель.Область(сч, 16).Текст);
		Стр17 = СокрЛП(ЛистЭксель.Область(сч, 17).Текст);
...
Функция ПолучитьЗначениеВЯчейкеОбщая(Лист, Строка, Колонка)
	...
	ЗначениеВЯчейке = СокрЛП(Лист.Область(Строка, Колонка).Текст);
...
Процедура ЗагрузитьОбщая()
	...
	Для сч = 4 По ЛистЭксель.ВысотаТаблицы Цикл // Самую первую пропускаем
...
Процедура ЗаполнитьОбщая()
	...
	Если ТаблУниверсальная.Количество() = 0 Или ЗагружатьВЗаполненныеТаблицы Тогда //веденеев 14.08.2017 - возможность не загружать данные, если они уже загружались
		...
		НачатьПоискФайлов(Новый ОписаниеОповещения("ЗаполнитьОбщаяЗавершение", ЭтотОбъект), ИмяФайла, "*.xls");
...
&НаКлиенте
Процедура ЗаполнитьОбщаяЗавершение(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	
	МассФайлов = НайденныеФайлы;
...
&НаКлиенте
Асинх Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РезультатАсинх = Ждать ДиалогОткрытияФайла.ВыбратьАсинх(); 
	Если Не РезультатАсинх = Неопределено Тогда
		ИмяФайла = РезультатАсинх[0];
	Иначе
		Возврат;
	КонецЕсли;
