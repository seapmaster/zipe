...
&НаКлиенте
Асинх Процедура ПутьКПрограммеКонвертацииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если НЕ Ждать ФайловыеФункцииСлужебныйКлиент.РасширениеРаботыСФайламиПодключено() Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура ОбновитьСостояние()
	...
	МассивУстройств = Ждать ФайловыеФункцииСлужебныйКлиент.ПолучитьУстройства();
