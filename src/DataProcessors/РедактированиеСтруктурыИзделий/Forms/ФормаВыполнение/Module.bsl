...
Процедура НачатьКопированиеВФоне()
	...
	ФоновоеЗадание = ФоновыеЗадания.Выполнить("СтруктураЗаказа.СкопироватьЭлементыСтруктуры", МассивПараметров, УникальныйИдентификатор, "Копирование элементов структуры кораблей");	
...
Процедура ВыполнитьПеренос()
	...
	РезультатВыполнения = СтруктураЗаказаВызовСервера.ПеренестиЭлементыСтруктуры(СписокСсылокИсточников.ВыгрузитьЗначения(), СсылкаПриемник);
