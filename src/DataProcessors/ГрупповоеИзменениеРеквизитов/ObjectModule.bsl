...
Функция ТекстЗапроса(ТипыИзменяемыхОбъектов) Экспорт
	...
	Если НЕ ВыбранныйАлгоритм.Пустая() И ОбъектыМетаданных.Количество() = 1 Тогда
		
		Если ЗначениеЗаполнено(ВыбранныйАлгоритм.ТекстЗапросаВиртуальныхТаблиц) Тогда
			Результат = ВыбранныйАлгоритм.ТекстЗапросаВиртуальныхТаблиц + "
			|;
			|" + Результат;
		КонецЕсли; 
		...
		Результат = Результат + "
		|ГДЕ
		|	Ссылка В (" +ВыбранныйАлгоритм.ТекстЗапросаФильтра + ")";
	КонецЕсли; 
