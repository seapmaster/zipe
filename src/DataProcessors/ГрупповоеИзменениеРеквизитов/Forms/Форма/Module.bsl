...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ 16.11.2017 Веденеев П. //вызов обработки из формы списка справочника
	Параметры.Свойство("ВидыИзменяемыхОбъектов", ВидыИзменяемыхОбъектов);
	
	НастройкиКомпоновки = Неопределено;
	
	Если Параметры.Свойство("НастройкиКомпоновки", НастройкиКомпоновки) Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ВидыИзменяемыхОбъектов) Тогда
		
		ВыбранныеОбъекты = СтрРазделить(ВидыИзменяемыхОбъектов, ",", Ложь);
		ВидыИзменяемыхОбъектов = "";
		
		НастройкиКомпоновки = КомпоновщикНастроек.Настройки;
		
		ВидИзменяемыхОбъектовВыборСделан(ВыбранныеОбъекты, Неопределено);
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Прервать" Тогда
		...
		Если Не РезультатФоновогоЗадания.Статус = "Выполнено" Тогда
...
Функция ЕстьНастроенныеОтборы()
	Для Каждого ЭлементОтбора Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если НЕ Объект.ВыбранныйАлгоритм.Пустая() Тогда
		Возврат Истина;
	КонецЕсли; 
	Возврат Ложь;
КонецФункции
...
Процедура ИзменитьОбъекты()
	...
	//+++ 13.12.2017 Добавление подробной строки состостояния
	...
	Иначе
		...
		Иначе
			...
			//Новый код:
			ОбщееКоличество = КоличествоОбъектовДляОбработки * НетранзакционнаяПорцияПолученияДанныхПроцент();
			Если ОбщееКоличество > 100000 Тогда
				Делитель = 10000;
			ИначеЕсли ОбщееКоличество > 10000 Тогда
				Делитель = 1000;
			Иначе
				Делитель = 100;
			КонецЕсли; 
			РазмерПорции = Окр(ОбщееКоличество / Делитель);
			//--- 13.12.2017 Добавление подробной строки состостояния
		...
		//Новый код:
		УправлениеИнтерфейсом.ВывестиТекущееСостояние(НСтр("ru = 'Обрабатываются элементы...'"),ВремяНачалаОбработки,0,КоличествоОбъектовДляОбработки);
		//--- 13.12.2017 Добавление подробной строки состостояния
	...
	//+++ 13.12.2017 Добавление подробной строки состостояния
	ТекущееСостояниеИзменения.Вставить("ВремяНачалаОбработки", ВремяНачалаОбработки);
	//--- 13.12.2017 Добавление подробной строки состостояния
...
Процедура ИзменитьПорциюОбъектов()
	...
	ДлительнаяОперация = ИзменитьНаСервере(ТекущееСостояниеИзменения.ОстанавливатьИзменениеПриОшибке);
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	//Новый код:
	УправлениеИнтерфейсом.ВывестиТекущееСостояние(НСтр("ru = 'Обрабатываются элементы...'"),ТекущееСостояниеИзменения.ВремяНачалаОбработки,ТекущееСостояниеИзменения.ТекущаяПозиция,ТекущееСостояниеИзменения.КоличествоОбъектовДляОбработки);
	//--- 13.12.2017 Добавление подробной строки состостояния
...
Функция ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	СтруктураПараметров.Вставить("ТипОперации", ОбработкаОбъект.ТипОперации);
	СтруктураПараметров.Вставить("КодАлгоритма", КодАлгоритма);
	СтруктураПараметров.Вставить("РежимВыполнения", ОбработкаОбъект.РежимВыполнения);
	СтруктураПараметров.Вставить("ВариантЗаписиОбъекта", ОбработкаОбъект.ВариантЗаписиОбъекта);
	...
	Если Не Объект.ИзменятьВТранзакции Или Не ПодсистемаСуществует("СтандартныеПодсистемы.БазоваяФункциональность") Тогда
		...
		ОбработкаОбъект.ИзменитьОбъекты(СтруктураПараметров, АдресХранилища);
		РезультатФоновогоЗадания = Новый Структура("Статус, АдресРезультата", "Выполнено", АдресХранилища);
	Иначе
		...
		ПараметрыЗадания.Вставить("ИмяМетода", "ИзменитьОбъекты");
...
Процедура ПриЗакрытииФормыВыбранныхОбъектов(Настройки, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Настройки) = Тип("Структура") Тогда
		Для каждого Стр Из Настройки Цикл
			Если Стр.Ключ = "Настройки" Тогда
				КомпоновщикНастроек.ЗагрузитьНастройки(Стр.Значение);
			ИначеЕсли Стр.Ключ = "Алгоритм" Тогда
				Объект.ВыбранныйАлгоритм = Стр.Значение;
			КонецЕсли; 
			ОбновитьНадписи();
		КонецЦикла; 
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ВидыИзменяемыхОбъектов) Тогда
		...
		Если НЕ ОбработкаОбъект.ВыбранныйАлгоритм.Пустая() Тогда
			Для каждого ПараметрФильтрации Из ОбработкаОбъект.ВыбранныйАлгоритм.ПараметрыЗапроса Цикл
				КомпоновщикНастроекКомпоновкиДанных.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра(ПараметрФильтрации.ИмяПараметра, ПараметрФильтрации.ЗначениеПараметра);
			КонецЦикла; 
		КонецЕсли;
...
Процедура ЗагрузитьМетаданныеОбъекта(СохранитьТекущиеНастройкиИзменения = Ложь, СохраненныеНастройки = Неопределено)
	...
	//++ 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
	ЗаполнитьРеквизитыОбъекта(ЗаблокированныеРеквизиты, НередактируемыеРеквизиты, ФильтруемыеРеквизиты, ОбщиеРеквизитыОбъектов.Реквизиты, ОбщиеРеквизитыОбъектов.ТабличныеЧасти);
	//-- 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
...
&НаСервере
Процедура ЗаполнитьРеквизитыОбъекта(БлокируемыеРеквизиты, НеРедактируемые, ФильтруемыеРеквизиты, ДоступныеРеквизиты, ДоступныеТабличныеЧасти)
	...
	//++ 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
	РеквизитыОбъектаАлгоритм.ПолучитьЭлементы().Очистить();
	//-- 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
	...
	//++ 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
	ДобавитьТабличныеЧастиВНабор(БлокируемыеРеквизиты, НеРедактируемые, ФильтруемыеРеквизиты, ДоступныеТабличныеЧасти, ОбъектМетаданных);
	//-- 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
...
//++ 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
&НаСервере
Процедура ДобавитьТабличныеЧастиВНабор(БлокируемыеРеквизиты, НеРедактируемые, ФильтруемыеРеквизиты, ДоступныеТабличныеЧасти, ОбъектМетаданных)
	
	РеквизитыОбъектаАлгоритмЭлементы = РеквизитыОбъектаАлгоритм.ПолучитьЭлементы();
	...
	Для Каждого ОписаниеТабличнойЧасти Из ОбъектМетаданных.ТабличныеЧасти Цикл
		...
		// фильтры табличных частей
		Если Нередактируемые.Найти(ОписаниеТабличнойЧасти.Имя + ".*") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ФильтруемыеРеквизиты.Найти(ОписаниеТабличнойЧасти.Имя + ".*") <> Неопределено Тогда
			Продолжить;
		...
		ДоступныеДляИзмененияРеквизиты = ДоступныеДляИзмененияРеквизиты(ОписаниеТабличнойЧасти, Нередактируемые,
		ФильтруемыеРеквизиты, ДоступныеТабличныеЧасти[ОписаниеТабличнойЧасти.Имя]);
		...
		ТабличнаяЧастьОбъектаАлгоритм = РеквизитыОбъектаАлгоритмЭлементы.Добавить();
		
		ТабличнаяЧастьОбъектаАлгоритм.Имя = ОписаниеТабличнойЧасти.Имя;
		ТабличнаяЧастьОбъектаАлгоритм.Представление = ОписаниеТабличнойЧасти.Представление();
		ТабличнаяЧастьОбъектаАлгоритм.ВидОперации = 3;
		
		ТабличнаяЧастьОбъектаАлгоритмСтроки = ТабличнаяЧастьОбъектаАлгоритм.ПолучитьЭлементы();
		
		Для каждого РеквизитТабличнойЧасти Из ДоступныеДляИзмененияРеквизиты Цикл
			
			РеквизитТабличнойЧастиОбъектаАлгоритм = ТабличнаяЧастьОбъектаАлгоритмСтроки.Добавить();
			РеквизитТабличнойЧастиОбъектаАлгоритм.Имя = РеквизитТабличнойЧасти.Имя;
			РеквизитТабличнойЧастиОбъектаАлгоритм.Представление = РеквизитТабличнойЧасти.Представление();
			РеквизитТабличнойЧастиОбъектаАлгоритм.ВидОперации = 1;
			РеквизитТабличнойЧастиОбъектаАлгоритм.ИмяТабличнойЧасти = ОписаниеТабличнойЧасти.Имя + ".";
...
Процедура ДобавитьРеквизитыВНабор(НаборыРеквизитов, ОбъектМетаданных)
	...
	Для Каждого ОписаниеРеквизита Из Реквизиты Цикл
		...
		//++ 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
		РеквизитОбъектаАлгоритм = РеквизитыОбъектаАлгоритмЭлементы.Добавить();
		ЗаполнитьЗначенияСвойств(РеквизитОбъектаАлгоритм, РеквизитОбъекта);
		//-- 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
...
Процедура ПерейтиКНастройкеОтбора()
	Если Не ПустаяСтрока(ВидыИзменяемыхОбъектов) Тогда
		...
		Новый Структура("ВыбранныеТипы, Настройки, Алгоритм", ВидыИзменяемыхОбъектов, КомпоновщикНастроек.Настройки, Объект.ВыбранныйАлгоритм), , , , , ОписаниеОповещения);
	КонецЕсли;
КонецПроцедуры
...
Процедура РеквизитыОбъекта1НачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	ПараметрыПеретаскивания.Значение = "Объект." + Элемент.ТекущиеДанные.ИмяТабличнойЧасти + Элемент.ТекущиеДанные.Имя;
	
КонецПроцедуры

//++ 21.11.2017 Веденеев П. //изменен механизм заполнения реквизитов для произвольного алгоритма (создано отдельное дерево, добавлены табличные части)
&НаКлиенте
Процедура РеквизитыОбъектаАлгоритмВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	КодАлгоритма = КодАлгоритма + " " + "Объект." + Элемент.ТекущиеДанные.ИмяТабличнойЧасти + Элемент.ТекущиеДанные.Имя;
