...
&НаКлиенте
Процедура СохранитьАрхивВФайлНаКлиенте(Результат) 
	...
	ДанныеХранилища.Записать(ПутьФайлаJSON);	
	...
	Для Каждого СсылкаНаФайл из МассивФайловКартинок Цикл
		...
		Если Счетчик = 50 Или ИндексТекущегоЭлемента = ОбщееКоличество - 1 Тогда
			...
			Для Каждого ПрисоединенныйФайлСтруктура из СтруктураФайлов Цикл
				...
				ДанныеХранилища.Записать(ПутьКВременномуФайлу); 		
...
&НаКлиенте
Процедура УдалитьВременныеФайлы(МассивДляУдаления)
