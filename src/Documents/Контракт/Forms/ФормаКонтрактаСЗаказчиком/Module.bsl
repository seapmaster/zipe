...
Асинх Функция ПолучитьКаталогВыгрузки()
	...
	РезультатВыбрать = Ждать ДиалогВыбораФайла.ВыбратьАсинх();
...
&НаКлиенте
Асинх Процедура ВыгрузкаТКП(Команда)
	...
	ВыгрузитьФайлWord("Контракт_ПисьмоТКПRus", ИмяФайла + " Rus.doc", ПараметрыТКП);
	ВыгрузитьФайлWord("Контракт_ПисьмоТКПEng", ИмяФайла + " Eng.doc", ПараметрыТКП);
...
Процедура ВыгрузитьФайлExcel(ИмяФайла, ПараметрыТКП)
	...
	ТабличныйДокумент = РаботаСФайламиOffice.ПодготовитьТабличныйДокументКонтрактПриложениеТКП(СпецификацияТКП, ПараметрыТКП);
	Если ТабличныйДокумент = Неопределено Тогда
		Возврат;
	КонецЕсли; // Если ТабличныйДокумент = Неопределено Тогда
	
	Попытка
		ТабличныйДокумент.ЗаписатьАсинх(ИмяФайла, ТипФайлаТабличногоДокумента.XLS);
	Исключение
...
Функция ПолучитьПараметрыТКП()
	...
	// Важен порядок!
	// т.к. параметры обновляются через СтрЗаменить()
...
Процедура ВыгрузитьПисьмоФСВТС(ИмяФайла, ПараметрыФСВТС)
	...
	АдресХранилища = РаботаСФайламиOffice.ПодготовитьПисьмоФСВТСКонтракт(ПараметрыПисьма);
...
Процедура ВыгрузитьПриложениеКПисьмуФСВТС(ИмяФайла, ПараметрыФСВТС)
	...
	ТабличныйДокумент = РаботаСФайламиOffice.ПодготовитьТабличныйДокументКонтрактПриложениеКПисьмуФСВТС(Объект.Дата, Объект.Валюта, Объект.Контрагент, Объект.ПредметыСнабжения, ПараметрыФСВТС);
	Если ТабличныйДокумент = Неопределено Тогда
		Возврат;
	КонецЕсли; // Если ТабличныйДокумент = Неопределено Тогда
