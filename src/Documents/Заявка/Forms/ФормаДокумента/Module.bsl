...
Процедура ОбработчикВыбораКолонокСпецификации(МассивКолонок, ДопПараметры) Экспорт
	...
	ДиалогОткрытияФайла.Показать(Новый ОписаниеОповещения("ОбработчикВыбораКолонокСпецификацииЗавершение", ЭтотОбъект, Новый Структура("ДиалогОткрытияФайла, ДопПараметры, МассивКолонок", ДиалогОткрытияФайла, ДопПараметры, МассивКолонок)));	

КонецПроцедуры

&НаКлиенте
Процедура ОбработчикВыбораКолонокСпецификацииЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	
	ДиалогОткрытияФайла = ДополнительныеПараметры.ДиалогОткрытияФайла;
	ДопПараметры = ДополнительныеПараметры.ДопПараметры;
	МассивКолонок = ДополнительныеПараметры.МассивКолонок;
	...
	Если ДопПараметры.Excel Тогда
		...
		ТабличныйДокумент.ЗаписатьАсинх(ПолноеИмяФайла + ?(Найти(ПолноеИмяФайла, ".xlsx") = 0, ".xlsx", ""), ТипФайлаТабличногоДокумента.XLSX);	
		
	ИначеЕсли ДопПараметры.Word Тогда
		
		ТабличныйДокумент.ЗаписатьАсинх(ПолноеИмяФайла + ?(Найти(ПолноеИмяФайла, ".docx") = 0, ".docx", ""), ТипФайлаТабличногоДокумента.DOCX);	
...
&НаКлиенте
Асинх Функция ПолучитьКаталогВыгрузки()
	...
	РезультатВыборка = Ждать ДиалогВыбораФайла.ВыбратьАсинх();
	Если РезультатВыборка = Неопределено Тогда
		Возврат Неопределено;
	Иначе
		Возврат РезультатВыборка[0] + "\";
	КонецЕсли;
	
КонецФункции

&НаКлиенте
Асинх Процедура ВыгрузкаВнешнихЦенДляФСВТС(Команда)
	
	Каталог = Ждать ПолучитьКаталогВыгрузки();
...
Процедура ВыгрузитьПисьмоФСВТС(ИмяФайла, ПараметрыФСВТС)
	...
	ПараметрыПисьма.Вставить("number_fsvtsaa", 			ПараметрыФСВТС.Получить("<НомерФСВТС>"));
	ПараметрыПисьма.Вставить("foreigncustomer", 		ПараметрыФСВТС.Получить("<ИностранныйЗаказчик>"));
	ПараметрыПисьма.Вставить("year_delivery", 			ПараметрыФСВТС.Получить("<ГодПоставки>"));
	ПараметрыПисьма.Вставить("delivery_conditions", 	ПараметрыФСВТС.Получить("<УсловияПоставки>"));
	ПараметрыПисьма.Вставить("AAНомерЗапросаAA", 		ПараметрыФСВТС.Получить("<НомерЗапроса>"));
	
	АдресХранилища = РаботаСФайламиOffice.ПодготовитьПисьмоФСТВСЗаявка(ПараметрыПисьма);
...
Процедура ВыгрузитьПриложениеКПисьмуФСВТС(ИмяФайла, ПараметрыФСВТС)
	...
	Попытка
		ТабличныйДокумент.ЗаписатьАсинх(ИмяФайла, ТипФайлаТабличногоДокумента.XLSX);
	Исключение
