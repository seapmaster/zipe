...
Процедура ВыгрузитьПриложениеКПисьмуФСВТС(ИмяФайла, ПараметрыФСВТС)
	...
	КоличествоСтрок = Объект.Спецификация.Количество(); 
	...
	ГодСтрокой = Формат(Год(Объект.Дата), "ЧГ=0");
	
	МассивЦенообразования = Новый Массив;
	
	ЕстьЦенаТекущегоГода   = Ложь;
	ЕстьЦенаСледующегоГода = Ложь;
	ЕстьЦенаГодаЧерез1     = Ложь;
	...
	Для каждого СтрокаПредметСнабжения Из Объект.Спецификация Цикл
		...
		Если ЗначениеЗаполнено(СтрокаПредметСнабжения.ЦенаТекущегоГода) Тогда
			Лист.Cells(Сч, 5).Value = СтрокаПредметСнабжения.ЦенаТекущегоГода;
			ЕстьЦенаТекущегоГода = Истина;
		КонецЕсли; 
		Если ЗначениеЗаполнено(СтрокаПредметСнабжения.ЦенаСледующегоГода) Тогда
			Лист.Cells(Сч, 6).Value = СтрокаПредметСнабжения.ЦенаСледующегоГода;
			ЕстьЦенаСледующегоГода = Истина;
		КонецЕсли; 
		Если ЗначениеЗаполнено(СтрокаПредметСнабжения.ЦенаГодаЧерез1) Тогда
			Лист.Cells(Сч, 7).Value = СтрокаПредметСнабжения.ЦенаГодаЧерез1;
			ЕстьЦенаГодаЧерез1 = Истина;
		КонецЕсли; 
		Лист.Cells(Сч, 8).Value = СтрокаПредметСнабжения.ЦенаОСК;
		Лист.Cells(Сч, 9).Value = Заказчик + ", " + Строка(Объект.Валюта) + ", " + ГодСтрокой + ", " + УсловияПоставки;
	...
	// Удаляем колонки где нет ни одной цены
	УдаленаЦенаТекущегоГода   = Ложь;
	УдаленаЦенаСледующегоГода = Ложь;
	
	Если НЕ ЕстьЦенаТекущегоГода Тогда
		Лист.Range("E:E").Delete(-4159);
		УдаленаЦенаТекущегоГода = Истина;
	Иначе
		СтрокаПункты = СтрокаПункты + "5";
	КонецЕсли; 
	
	Если НЕ ЕстьЦенаСледующегоГода Тогда
		Если УдаленаЦенаТекущегоГода Тогда
			Лист.Range("E:E").Delete(-4159);
		Иначе
			Лист.Range("F:F").Delete(-4159);
		КонецЕсли; 
		УдаленаЦенаСледующегоГода = Истина;
	Иначе
		СтрокаПункты = СтрокаПункты + ?(ЕстьЦенаТекущегоГода, ", 6", "5");
	КонецЕсли; 
	
	Если НЕ ЕстьЦенаГодаЧерез1 Тогда
		Если УдаленаЦенаТекущегоГода И УдаленаЦенаСледующегоГода Тогда // Обе удалены
			Лист.Range("E:E").Delete(-4159);
		ИначеЕсли НЕ УдаленаЦенаТекущегоГода И НЕ УдаленаЦенаСледующегоГода Тогда // Ни одна
			Лист.Range("G:G").Delete(-4159);
		Иначе 															// Одна из 2х
			Лист.Range("F:F").Delete(-4159);
		КонецЕсли; 
	Иначе
		СтрокаПункты = СтрокаПункты + ?(ЕстьЦенаТекущегоГода И ЕстьЦенаСледующегоГода, ", 7", ?(НЕ ЕстьЦенаТекущегоГода И НЕ ЕстьЦенаСледующегоГода, "5",", 6")); 
	КонецЕсли; 
	
	СледКолонка = ?(ЕстьЦенаТекущегоГода, 1, 0) + ?(ЕстьЦенаСледующегоГода, 1, 0) + ?(ЕстьЦенаГодаЧерез1, 1, 0);
	
	ПозицияДопСтроки = 5;
	ДопСтрока = СтрЗаменить("(к пункту 8)", "8", Строка(ПозицияДопСтроки + СледКолонка));
	
	Лист.Cells(4, ПозицияДопСтроки   +   СледКолонка).Value = "Цена для формирования стоимости предложения по п. " + СтрокаПункты;
	Лист.Cells(4, ПозицияДопСтроки + 1 + СледКолонка).Value = "Заказчик, валюта цены, год цены, условия поставки " + ?(ДопСтрока= "", "", Символы.ПС) + ДопСтрока;
	
	// Перенумерация колонок
	ИндексПервойКолонки = 5;
	Пока ИндексПервойКолонки > 0 Цикл
		Лист.Cells(5, ИндексПервойКолонки).Value = ИндексПервойКолонки;
		ИндексПервойКолонки = ИндексПервойКолонки + 1;
		Если НЕ ЗначениеЗаполнено(Лист.Cells(4, ИндексПервойКолонки).Value) Тогда
	...
	Текст = "Начальник отдела ценообразования: " + ПолучитьНачальникаОтделаЦенообразования();
	Лист.Cells(Сч + 1, 1).Value = Текст;
