...
Функция ДанныеОбновлены(Сообщить = Ложь, ОбъектыРасширений = Ложь) Экспорт
	...
	Если ОбъектыРасширений
	   ...
	   И Справочники.ВерсииРасширений.УстановленныеРасширенияДоступны() Тогда
		...
		Если Не ИдентификаторыОбъектовТекущейВерсииРасширенийЗаполнены() Тогда
...
Процедура ВыполнитьОбновлениеДанных(ЕстьИзменения, ЕстьУдаленные, РежимОбновления,
	...
	Если ОбъектыРасширений
	   И ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения)
	   И Не Справочники.ВерсииРасширений.УстановленныеРасширенияДоступны() Тогда
		
		ТекстОшибки =
			НСтр("ru = 'Невозможно обновить идентификаторы объектов расширений
...
//
// Параметры:
//  Идентификаторы - Массив - значения либо типа СправочникСсылка.ИдентификаторыОбъектовМетаданных,
//                    либо типа СправочникСсылка.ИдентификаторыОбъектовРасширений - идентификаторы
//                    объектов метаданных конфигурации или расширения.
//
// Возвращаемое значение:
//  Соответствие - со свойствами:
//   * Ключ - переданный идентификатор,
//   * Значение - ОбъектМетаданных - для типа СправочникСсылка.ИдентификаторыОбъектовМетаданных.
...
//
Функция ОбъектыМетаданныхПоИдентификаторам(Идентификаторы) Экспорт
	
	ОбъектыРасширений = ЭтоОбъектРасширений(Идентификаторы[0]);
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Если РезультатПроверки.НеСоответствует Тогда
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				Если ОбъектыРасширений И Не Свойства.ПометкаУдаления Тогда
				...
				ИначеЕсли РезультатПроверки.КлючОбъектаМетаданных = Неопределено Тогда
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Идентификатору ""%1""
						           ...
						           |""%3"".'"),
						Строка(Свойства.Ссылка), НазваниеСправочника, Свойства.ПолноеИмя);
				Иначе
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Идентификатору ""%1""
						           |найденному в справочнике ""%2"",
						           |соответствует удаленный объект метаданных.'"),
						Строка(Свойства.Ссылка), НазваниеСправочника);
				КонецЕсли;
			Иначе
				ОписаниеОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Идентификатору ""%1""
					...
					Строка(Свойства.Ссылка), НазваниеСправочника, РезультатПроверки.ОбъектМетаданных.ПолноеИмя());
			КонецЕсли;
		...
		Если Не ОбъектыРасширений И Свойства.ПометкаУдаления Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идентификатор ""%1""
		...
		ОбъектыМетаданныхИдентификаторов.Вставить(Свойства.Ссылка, РезультатПроверки.ОбъектМетаданных);
...
Функция ИдентификаторОбъектаМетаданныхПоПолномуИмени(ПолноеИмяОбъектаМетаданных) Экспорт
	...
	ДоступныИдентификаторыОбъектовРасширений =
		...
		И Справочники.ВерсииРасширений.УстановленныеРасширенияДоступны();
...
Функция ОбъектМетаданныхПоИдентификатору(Идентификатор) Экспорт
	...
	Возврат ОбъектыМетаданныхПоИдентификаторам(Идентификаторы).Получить(Идентификатор);
...
Процедура ОбновитьДанные(ЕстьИзменения, ЕстьУдаленные, РежимОбновления,
	...
	Если ОбъектыРасширений Или РежимОбновления <> "Полный" Или МонопольныйРежим() Тогда
		СнятьМонопольныйРежим = Ложь;
	Иначе
	...
	Попытка
		...
		ТолькоПроверка = (РежимОбновления = "ТолькоПроверка")
			Или Не ОбъектыРасширений И РежимОбновления <> "Полный" И ЕстьКритичныеИзменения;
...
Функция СвойстваОбъектовМетаданных(ОбъектыРасширений, СвойстваКоллекций = Неопределено)
	...
	Если ОбъектыРасширений И Не ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения) Тогда
		Возврат СвойстваОбъектовМетаданных;
	КонецЕсли;
	
	Для каждого СвойстваКоллекции Из СвойстваКоллекций Цикл
		ДобавитьСвойстваОбъектовМетаданных(Метаданные[СвойстваКоллекции.Имя], СвойстваКоллекции,
			СвойстваОбъектовМетаданных, ОбъектыРасширений);
	КонецЦикла;
