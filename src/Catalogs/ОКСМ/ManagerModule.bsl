...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Иначе
		...
		ШаблонЗапроса = "
			...
			|	ЕСТЬNULL(ОКСМ.Код, Классификатор.Код)                   КАК Код,
			|	ЕСТЬNULL(ОКСМ.Наименование, Классификатор.Наименование) КАК Наименование,
			|	ЕСТЬNULL(ОКСМ.ПометкаУдаления, ЛОЖЬ)                    КАК ПометкаУдаления,
			|
			|	ВЫБОР КОГДА ОКСМ.Ссылка ЕСТЬ NULL ТОГДА 
			|		%2 
			|	ИНАЧЕ 
			|		%3
			...
			|	Классификатор
			|ПО
			|	Классификатор.Код = ОКСМ.Код
			|	И Классификатор.Наименование = ОКСМ.Наименование
			|ГДЕ 
			|	(ОКСМ.%1 ПОДОБНО &СтрокаПоиска ИЛИ Классификатор.%1 ПОДОБНО &СтрокаПоиска)
			|	И (" + СтрЗаменить(ШаблонОтбора, "%1", "Классификатор") + ")
			|	И (" + СтрЗаменить(ШаблонОтбора, "%1", "ОКСМ") + ")
			|
			|УПОРЯДОЧИТЬ ПО
			|	ЕСТЬNULL(ОКСМ.%1, Классификатор.%1)
			|";
	КонецЕсли;
	...
	Для Каждого ДанныеПоля Из ИменаПолей.СписокПолей Цикл
		...
		Запрос.Текст = СтрЗаменить(ТекстЗапроса, "%3", СтрЗаменить(ДанныеПоля.ШаблонПредставления, "%1", "ОКСМ"));
...
//          * Ссылка             - СправочникаСсылка.ОКСМ - реквизит найденной страны.
//    Неопределено - страна не найдена.
//
Функция ДанныеОКСМ(Знач КодСтраны = Неопределено, Знач Наименование = Неопределено) Экспорт
	...
	НормализованныйКод = КодОКСМ(КодСтраны);
	...
	Результат = Новый Структура("Ссылка, Код, Наименование, ПолноеНаименованиеСтраны, КодАльфа2, КодАльфа3");
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	Ссылка, Код, Наименование, ПолноеНаименованиеСтраны, КодАльфа2, КодАльфа3
	|ИЗ
	|	Справочник.ОКСМ
	|ГДЕ
	|	" + УсловиеПоиска + "
...
Функция ДанныеКлассификатораСтранМираПоКоду(Знач Код, ТипКода = "КодСтраны") Экспорт
	...
	Если СтрСравнить(ТипКода, "Альфа2") = 0 Тогда
		СтрокаДанных = ДанныеКлассификатора.Найти(ВРег(Код), "БуквенныйКодАльфа2");
	ИначеЕсли СтрСравнить(ТипКода, "Альфа3") = 0 Тогда
		СтрокаДанных = ДанныеКлассификатора.Найти(ВРег(Код), "БуквенныйКодАльфа3");
	Иначе
		СтрокаДанных = ДанныеКлассификатора.Найти(КодОКСМ(Код), "Код");
	КонецЕсли;
	
	Если СтрокаДанных=Неопределено Тогда
		Результат = Неопределено;
	Иначе
		Результат = Новый Структура("Код, Наименование, ПолноеНаименованиеСтраны, БуквенныйКодАльфа2, БуквенныйКодАльфа3");
...
//
// Возвращаемое значение:
//     СправочникСсылка.ОКСМ - ссылка на созданный элемент.
//
Функция СсылкаПоДаннымКлассификатора(Знач Отбор, Знач ДополнительныеДанные = Неопределено) Экспорт
	...
	ДанныеПоиска = ДанныеОКСМ(ДанныеПоиска.Код, ДанныеПоиска.Наименование);
	Результат = ДанныеПоиска.Ссылка;
	Если Не ЗначениеЗаполнено(Результат) Тогда
		ОбъектСтраны = Справочники.ОКСМ.СоздатьЭлемент();
...
Функция ТаблицаКлассификатора() Экспорт
	Макет = Справочники.ОКСМ.ПолучитьМакет("Классификатор");
...
Функция ЕстьПравоДобавления() Экспорт
	Возврат ПравоДоступа("Добавление", Метаданные.Справочники.ОКСМ);
КонецФункции
...
Функция ПоляПоиска()
	Результат = Новый Массив;
	СписокПолей = Справочники.ОКСМ.ПустаяСсылка().Метаданные().ВводПоСтроке;
...
//
Функция КодОКСМ(Знач КодСтраны)
