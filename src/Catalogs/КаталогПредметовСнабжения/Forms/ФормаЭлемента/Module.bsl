...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда   
		...
		//+ 13.03.2017 11:39:51 Савинов Александр Задача: ОПЗ 2
		...
		// ++ 06.09.2018 11:43:36 Базунов Д.А. Задача: 
		ЗаполнитьКодыINCAT();
		// -- 06.09.2018 11:43:36 Базунов Д.А. Задача:
		
		//- 13.03.2017 11:39:55 Савинов Александр Задача: ОПЗ 2
		
		ТолькоПросмотр = ОбщиеФункцииСервер.ПроверкаВозможностиИзменения(ВернутьМассивПоставщиков());
		
		РольСБД = РольДоступна("СпециалистПоВедениюБД"); // БазуновДА 26.06.2018 СБД дожен изменять каталог ПС
	...
	ЕстьСтрокиСпецификация = Спецификация.Количество() > 0;
	Элементы.СпецификацияПереместитьПозициюВверх.Доступность = ЕстьСтрокиСпецификация;
	Элементы.СпецификацияПереместитьПозициюВниз.Доступность = ЕстьСтрокиСпецификация;
	...
	Если Не РольДоступна("ПолныеПрава") И (ЭтоПредставительПроектантаКорабля ИЛИ ЭтоПредставительСтроителяКорабля ИЛИ ЭтоПредставительКонтрагента) Тогда
		// ++ 17.07.2018 16:47:18 Базунов Д.А. Задача: 
		// Если стоит ИспользоватьБизнесПроцессыДляКорректировкиСправочников то открываем доступность реквизитов
		// -- 17.07.2018 16:47:18 Базунов Д.А. Задача:
		...
		// Базунов 24.03.2018
		//Элементы.ГруппаЦеныВнешние.Видимость 	= Ложь;
		//Элементы.ГруппаЦеныВнутренние.Видимость = Ложь;
	
	КонецЕсли;
	
	//+ 22.11.2017 13:36:38 Базунов Д.А. Задача: 
	ДобавитьИЗаполнитьГородВПоставщиках();
	//- 22.11.2017 13:36:38 Базунов Д.А. Задача: 
	
	//++ 17.11.2017 Веденеев П. //изменен формат хранения ФНН
	ФНН = Объект.ФНН;
	//-- 17.11.2017 Веденеев П. //изменен формат хранения ФНН
	...
	//+ 17.04.2018 16:43:38 Базунов Д.А. Задача: 
	...
	// ++ 09.07.2018 11:16:41 Базунов Д.А. Задача: 
	...
	// ++ 21.05.2018 11:35:42 Базунов Д.А. Задача: 
...
Процедура ДобавитьИЗаполнитьГородВПоставщиках()
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("ПоставщикиГород", Новый ОписаниеТипов("Строка"), "Объект.ИзготовителиИПоставщики", "Город"));
	ИзменитьРеквизиты(МассивРеквизитов);
	
	КолонкаГород = Элементы.Вставить("ИзготовителиИПоставщикиГород", Тип("ПолеФормы"), Элементы.ИзготовителиИПоставщики, Элементы.ИзготовителиИПоставщикиПоставщик);
	КолонкаГород.Вид = ВидПоляФормы.ПолеВвода;
	КолонкаГород.ПутьКДанным = "Объект.ИзготовителиИПоставщики.ПоставщикиГород";
...
Процедура ОбновитьГородВПоставщиках()
	...
	Если Объект.ИзготовителиИПоставщики.Количество() > 0 Тогда
		...
		|	КаталогПредметовСнабженияИзготовителиИПоставщики.НомерСтроки КАК НомерСтроки
		|ИЗ
		|	Справочник.КаталогПредметовСнабжения.ИзготовителиИПоставщики КАК КаталогПредметовСнабженияИзготовителиИПоставщики
		|ГДЕ
		|	КаталогПредметовСнабженияИзготовителиИПоставщики.Ссылка = &Ссылка
		|	И НЕ КаталогПредметовСнабженияИзготовителиИПоставщики.Контрагент.Представление ЕСТЬ NULL";
		...
		Пока Выборка.Следующий() Цикл
			...
			ПоискАдрес = Выборка.Контрагент.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Тип", Перечисления.ТипыКонтактнойИнформации.Адрес));
			Если ПоискАдрес.Количество() > 0 Тогда
				ТекАдрес = ПоискАдрес[0];
				XDTOКонтактная = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияИзXML(ТекАдрес.ЗначенияПолей, Перечисления.ТипыКонтактнойИнформации.Адрес);
				Объект.ИзготовителиИПоставщики[Выборка.НомерСтроки-1].ПоставщикиГород = УправлениеКонтактнойИнформациейСлужебный.ПредставлениеНаселенногоПункта(XDTOКонтактная);
			КонецЕсли;
		...
		//		ТекАдрес = ПоискАдрес[0];
		//		XDTOКонтактная = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияИзXML(ТекАдрес.ЗначенияПолей, Перечисления.ТипыКонтактнойИнформации.Адрес);
		//		ТекЭлемент.ПоставщикиГород = УправлениеКонтактнойИнформациейСлужебный.ПредставлениеНаселенногоПункта(XDTOКонтактная);
		//	КонецЕсли;
		//	
		//КонецЦикла; 
	КонецЕсли; 
...
// ++ 06.09.2018 11:43:57 Базунов Д.А. Задача: 
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи) 
	...
	//+ 14.03.2017 11:34:59 Савинов Александр Задача: Оптимизация
...
#Область СтарыйКод
	...
	//	|ГДЕ
	//	|	(КаталогПС.ДокументНаПоставку = &ДокументНаПоставку И &ДокументНаПоставку <> """"
	//	|	ИЛИ КаталогПС.Обозначение = &Обозначение И &Обозначение <> """")
	//	|	И КаталогПС.Ссылка <> &Ссылка";
	//
	//Запрос.УстановитьПараметр("Обозначение", СокрЛП(ТекущийОбъект.Обозначение));
	//Запрос.УстановитьПараметр("ДокументНаПоставку", СокрЛП(ТекущийОбъект.ДокументНаПоставку));
	...
	//
	//	Сообщить("Уже имеются другие элементы с одинаковыми обозначениями или документами на поставку:",СтатусСообщения.Информация);
	//	
	//	Выборка = РезультатЗапроса.Выбрать();
	//	
	//	Пока Выборка.Следующий() Цикл
	...
	//		Сообщение.Текст = "" + Выборка.Наименование + ", обозначение: " + Выборка.Обозначение + ", документ на поставку: " + Выборка.ДокументНаПоставку;
	...
	//++ 13.03.2018 Веденеев П. //отключение непосредственного изменения данных
	// ++ 18.07.2018 15:51:42 Базунов Д.А. Задача: 
	...
	// Скорректируем при необходимости реквизиты для проверки
	...
	Если МодифицированностьСпецификации Тогда 
		
		ТаблицаСпецификации = Спецификация.Выгрузить();
		
		// Проверим корректность заполнения спецификации
		
		РезультатПроверкиСпецификации = ПроверитьСпецификацию(ТаблицаСпецификации);
		
		Если Не РезультатПроверкиСпецификации.ЗаполнениеКорректно Тогда
		
			Отказ = Истина;
			Сообщить(РезультатПроверкиСпецификации.Описание);
			Возврат;
		
		КонецЕсли;
		
		//-- 29.11.2017 Веденеев П. //проверка вхождения элемента в спецификации его составных частей
		// ++ 18.07.2018 15:52:03 Базунов Д.А. Задача: 
		//Если ПолучитьФункциональнуюОпцию("ИспользоватьБизнесПроцессыДляКорректировкиСправочников") И КоманднаяПанель.ПодчиненныеЭлементы.ФормаОбщаяКомандаАкцептоватьИзмененияВСправочнике.Видимость Тогда
		//	
		//	РезультатПроверки = ПроверитьНаличиеЗацикливания();
		//	
		//	Если РезультатПроверки.Ошибка Тогда
		//		
		//		Отказ = Истина;
		//		Сообщить(РезультатПроверки.Описание);
		//		Возврат;
		//		
		//	КонецЕсли;
		//	
		...
		ТекущийОбъект.ДополнительныеСвойства.Вставить("Спецификация", ТаблицаСпецификации);	
	...
	// ++ 06.09.2018 11:45:43 Базунов Д.А. Задача: 
	...
	Если СпецификацияИзменена() Тогда 
		...
		ТекущийОбъект.ДополнительныеСвойства.Вставить("ДобавитьПредметСнабжениявОчередьИзмененияСпецификации");	
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если МодифицированностьСпецификации И Не ПропуститьПроверкуЗаполненияСведенийОбИзмененияхСпецификации Тогда
	
		Отказ = Истина;
		
		ПередЗаписьюПриМодифицированностиСпецификации(ПараметрыЗаписи);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ПропуститьПроверкуЗаполненияСведенийОбИзмененияхСпецификации = Ложь;
	МодифицированностьСпецификации = Ложь;
	
	Если ПараметрыЗаписи.Свойство("Закрыть") Тогда
	
		Закрыть();	
		
	ИначеЕсли Не Элементы.СпецификацияПоказатьИсториюСпецификации.Видимость Тогда
		
		Элементы.СпецификацияПоказатьИсториюСпецификации.Видимость = Истина;	
...
// Процедура предназначена для заполнения таблицы спецификации при создании формы
//
&НаСервере
Процедура ЗаполнитьСпецификации()
	...
	Спецификация.Загрузить(РезультатЗапроса.Выгрузить());
	
	Элементы.СпецификацияПоказатьИсториюСпецификации.Видимость = Истина;

КонецПроцедуры // ЗаполнитьСпецификации()

// Процедура предназначена для установки модифицированности при изменении спецификации
//
&НаКлиенте
Процедура СпецификацияПриИзменении(Элемент = Неопределено, НомерПозицииМодифицированнойСтроки = Неопределено)
	...
	МодифицированностьСпецификации = Истина; // Для того, чтобы при записи элемента записать изменения спецификации 
	
	ЕстьСтрокиСпецификация = Спецификация.Количество() > 0;
	Элементы.СпецификацияПереместитьПозициюВверх.Доступность = ЕстьСтрокиСпецификация;
	Элементы.СпецификацияПереместитьПозициюВниз.Доступность = ЕстьСтрокиСпецификация;
	
	// Зафиксируем индекс модифицированной строки
	
	Если НомерПозицииУдаленнойСтрокиСпецификации > 0 Тогда
	
		НомерПозицииУдаленнойСтрокиСпецификации = 0;
		Возврат;	
	
	КонецЕсли;
	
	Если НомерПозицииМодифицированнойСтроки = Неопределено Тогда
	
		НомерПозицииМодифицированнойСтроки = Элементы.Спецификация.ТекущиеДанные.НомерПозиции;
		
	КонецЕсли;
	
	Если СписокНомеровПозицийМодифицированныхСтрокСпецификации.НайтиПоЗначению(НомерПозицииМодифицированнойСтроки) = Неопределено Тогда
	
		СписокНомеровПозицийМодифицированныхСтрокСпецификации.Добавить(НомерПозицииМодифицированнойСтроки);	
	
	КонецЕсли;	
	
КонецПроцедуры

// Процедура предназначена для добавления строк спецификации с заполнением номера позиции
//
&НаКлиенте
Процедура СпецификацияПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("СпецификацияДобавление", ЭтотОбъект);
	
	ОткрытьФорму("Справочник.КаталогПредметовСнабжения.Форма.ФормаПозицииСостава",, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
	
КонецПроцедуры

&НаКлиенте
Процедура СпецификацияДобавление(ВозЗнач, ДопПараметры = Неопределено) Экспорт
	...
	НоваяСтрока.НомерПозиции = Спецификация.Количество();
	
	ЗаполнитьЗначенияСвойств(НоваяСтрока, ВозЗнач);
	
	СпецификацияПриИзменении(, НоваяСтрока.НомерПозиции);

КонецПроцедуры // СпецификацияДобавление()
...
Процедура СпецификацияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПараметрыОткрытия.Вставить("НомерПозиции", ТекДанные.НомерПозиции); //++ 16.05.2018 Веденеев П. 
	ПараметрыОткрытия.Вставить("СоставляющаяЧасть", ТекДанные.СоставляющаяЧасть);
	ПараметрыОткрытия.Вставить("ЗИП", ТекДанные.ЗИП);
	ПараметрыОткрытия.Вставить("Тип", ТекДанные.Тип);
	ПараметрыОткрытия.Вставить("Количество", ТекДанные.Количество);
	ПараметрыОткрытия.Вставить("ОбозначениеСНК", ТекДанные.ОбозначениеСНК);
	ПараметрыОткрытия.Вставить("ПричинаВыбораДляЗИП", ТекДанные.ПричинаВыбораДляЗИП);
	ПараметрыОткрытия.Вставить("УказательНаЭлементСхемы", ТекДанные.УказательНаЭлементСхемы);
	ПараметрыОткрытия.Вставить("Взаимозаменяемость", ТекДанные.Взаимозаменяемость);
	ПараметрыОткрытия.Вставить("ПризнакПодбора", ТекДанные.ПризнакПодбора);
	ПараметрыОткрытия.Вставить("ОбозначениеЗаготовки", ТекДанные.ОбозначениеЗаготовки);
	ПараметрыОткрытия.Вставить("СведенияОбИзменениях", ТекДанные.СведенияОбИзменениях);
 	
	ОписаниеОповещения = Новый ОписаниеОповещения("СпецификацияОбработчикРедактирования", ЭтотОбъект);
	
	ОткрытьФорму("Справочник.КаталогПредметовСнабжения.Форма.ФормаПозицииСостава", ПараметрыОткрытия, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура СпецификацияОбработчикРедактирования(ВозЗнач, ДопПараметры = Неопределено) Экспорт
	...
	//++ 30.11.2017 Веденеев П. //проверка изменения данных спецификации
	ЕстьИзменения = ДанныеИзменены(ТекДанные, ВозЗнач);
	//-- 30.11.2017 Веденеев П. //проверка изменения данных спецификации
	
	ЗаполнитьЗначенияСвойств(ТекДанные, ВозЗнач);
	
	//++ 30.11.2017 Веденеев П. //проверка изменения данных спецификации
	Если ЕстьИзменения Тогда
		
		СпецификацияПриИзменении();
		
	КонецЕсли;
		
	//-- 30.11.2017 Веденеев П. //проверка изменения данных спецификации

КонецПроцедуры // СпецификацияОбработчикРедактирования()

//++ 30.11.2017 Веденеев П. //проверка изменения данных спецификации
&НаКлиенте
Функция ДанныеИзменены(СтарыеДанные, НовыеДанные)
	
	Результат = Ложь;
	
	Для каждого ЭлементДанных Из НовыеДанные Цикл
		
		Если Не СтарыеДанные[ЭлементДанных.Ключ] = ЭлементДанных.Значение Тогда
...
// Процедура предназначена для проверки заполнения сведений об изменениях в модифицированных строках спецификации и обязательных для заполнения полей
//
&НаКлиенте
Процедура ПередЗаписьюПриМодифицированностиСпецификации(ПараметрыЗаписи)
	
	МассивЗамечаний = Новый Массив;
	
	МассивМодифицированныхСтрокСНезаполненнымиСведениямиОбИзменениях = Новый Массив;
	
	Для каждого Строка Из Спецификация Цикл
		
		Если Не ЗначениеЗаполнено(Строка.СоставляющаяЧасть) Тогда
		
			МассивЗамечаний.Добавить("Не заполнена составная часть в строке " + Строка(Строка.НомерПозиции));
			МассивЗамечаний.Добавить(Символы.ПС);
		
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Строка.Тип) Тогда
		
			МассивЗамечаний.Добавить("Не заполнен тип в строке " + Строка(Строка.НомерПозиции));
			МассивЗамечаний.Добавить(Символы.ПС);
		...
		//
		//	МассивЗамечаний.Добавить("Не заполнено количество в строке " + Строка(Строка.НомерПозиции));
		//	МассивЗамечаний.Добавить(Символы.ПС);
		//
		//КонецЕсли;
		//
		//Если Строка.ЗИП И Не ЗначениеЗаполнено(Строка.ПричинаВыбораДляЗИП) Тогда
		//
		//	МассивЗамечаний.Добавить("Не заполнена причина выбора для ЗИП в строке " + Строка(Строка.НомерПозиции));
		//	МассивЗамечаний.Добавить(Символы.ПС);
		//
		//КонецЕсли;
	
		Если Не СписокНомеровПозицийМодифицированныхСтрокСпецификации.НайтиПоЗначению(Строка.НомерПозиции) = Неопределено И Не ЗначениеЗаполнено(Строка.СведенияОбИзменениях) Тогда
		
			МассивМодифицированныхСтрокСНезаполненнымиСведениямиОбИзменениях.Добавить(Строка.НомерПозиции);	
	...
	Если МассивЗамечаний.Количество() > 0 Тогда
		...
		МассивЗамечаний.Удалить(МассивЗамечаний.Количество() - 1);
		Сообщить(СтрСоединить(МассивЗамечаний));
		Возврат;
	
	КонецЕсли;
	
	Если МассивМодифицированныхСтрокСНезаполненнымиСведениямиОбИзменениях.Количество() > 0 Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПередЗаписьюПриМодифицированностиСпецификацииПродолжение", ЭтотОбъект, ПараметрыЗаписи);
		
		МассивТекстаВопроса = Новый Массив;
		
		МассивТекстаВопроса.Добавить("Не заполнены сведения об изменениях в измененных строках с номерами позиций: ");
		
		Для каждого ЭлементМассива Из МассивМодифицированныхСтрокСНезаполненнымиСведениямиОбИзменениях Цикл
		
			МассивТекстаВопроса.Добавить(ЭлементМассива);
			МассивТекстаВопроса.Добавить(", ");
		
		КонецЦикла;
		
		МассивТекстаВопроса.Удалить(МассивТекстаВопроса.Количество() - 1);
		
		МассивТекстаВопроса.Добавить(Символы.ПС);
		МассивТекстаВопроса.Добавить("Продолжить запись без заполнения сведений об изменениях?");
		
		ТекстВопроса = СтрСоединить(МассивТекстаВопроса);
		
		СписокКнопок = Новый СписокЗначений;
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, "Продолжить запись");
		СписокКнопок.Добавить(КодВозвратаДиалога.Нет, "Отменить запись");
		
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок,,, "Не заполнены сведения об изменениях в измененных строках");	
	
	Иначе
	
		ПередЗаписьюПриМодифицированностиСпецификацииПродолжение(КодВозвратаДиалога.Да, ПараметрыЗаписи);	
	
	КонецЕсли;

КонецПроцедуры // ПередЗаписьюПриМодифицированностиСпецификации()

// Процедура предназначена для записи после проверки заполнения сведений об изменениях в модифицированных строках спецификации 
//
&НаКлиенте
Процедура ПередЗаписьюПриМодифицированностиСпецификацииПродолжение(Ответ, ДопПараметры) Экспорт

	Если Ответ = КодВозвратаДиалога.Да Тогда
	
		ПропуститьПроверкуЗаполненияСведенийОбИзмененияхСпецификации = Истина;
		
		Записать(ДопПараметры);	
	
	КонецЕсли;		

КонецПроцедуры // ПередЗаписьюПриМодифицированностиСпецификацииПродолжение()

// Функция предназначена для проверки уникальности строк спецификации по ключу: Составляющая часть + Признак ЗИП
//
&НаСервереБезКонтекста
Функция ПроверитьСпецификацию(ТаблицаСпецификации)
	
	Результат = Новый Структура("ЗаполнениеКорректно, Описание", Ложь, "");

	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ТаблицаСпецификации.СоставляющаяЧасть,
	|	ТаблицаСпецификации.ЗИП,
	|	1 КАК Счетчик
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаСпецификации.СоставляющаяЧасть,                                                                           	
	|	ВТ_ТаблицаСпецификации.ЗИП
	|ИЗ
	|	ВТ_ТаблицаСпецификации КАК ВТ_ТаблицаСпецификации
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ТаблицаСпецификации.СоставляющаяЧасть,
	|	ВТ_ТаблицаСпецификации.ЗИП
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(ВТ_ТаблицаСпецификации.Счетчик) > 1";
	...
	МассивТекстаСообщения.Добавить("В спецификации содержатся строки с одинаковыми составными частями и признаком ЗИП:");
	МассивТекстаСообщения.Добавить(Символы.ПС);
	...
	Пока Выборка.Следующий() Цикл
		...
		МассивТекстаСообщения.Добавить(Строка(Выборка.СоставляющаяЧасть));
...
КонецФункции // ПроверитьСпецификацию()

//++ 29.11.2017 Веденеев П. //проверка вхождения элемента в спецификации его составных частей
&НаСервере
Функция ПроверитьНаличиеЗацикливания()
	
	Результат = Новый Структура("Ошибка, Описание", Ложь, "");
	
	СтрокиНеЗИП = Спецификация.НайтиСтроки(Новый Структура("ЗИП", Ложь));
	
	Если СтрокиНеЗИП.Количество() = 0 Тогда
		
		Возврат Результат;
		
	КонецЕсли;
	
	МассивСоставляющихЧастей = Новый Массив;
	
	Для каждого СтрокаНеЗИП Из СтрокиНеЗИП Цикл
		
		Если СтрокаНеЗИП.СоставляющаяЧасть = Объект.Ссылка Тогда
			
			Результат.Ошибка = Истина;
			Результат.Описание = "В спецификации предмета снабжения присутствует он же без признака ЗИП!";
			Возврат Результат;
			
		КонецЕсли;
		
		МассивСоставляющихЧастей.Добавить(СтрокаНеЗИП.СоставляющаяЧасть);
		
	КонецЦикла;
	
	Если СтруктураЗаказаСервер.ВходитВСпецификацииПредметовСнабжения(Объект.Ссылка, МассивСоставляющихЧастей) Тогда
		
		Результат.Ошибка = Истина;
		Результат.Описание = "Предмет снабжения входит в дерево спецификации одной из составляющих частей!";
...
// Процедура предназначена для изменения номеров позиции составляющих частей спецификации путем перемещения одной строки на позицию выше
//
&НаКлиенте
Процедура ПереместитьПозициюСпецификацииВверх(Команда)
	
	ТекущийНомерПозицииСпецификации = Элементы.Спецификация.ТекущиеДанные.НомерПозиции;
	
	Если ТекущийНомерПозицииСпецификации = 1 Тогда // Выше некуда
	
		Возврат;	
	
	КонецЕсли;
	
	ИзменитьПорядокСоставляющихЧастейСпецификации(ТекущийНомерПозицииСпецификации, "Вверх");
	
КонецПроцедуры

// Процедура предназначена для изменения номеров позиции составляющих частей спецификации путем перемещения одной строки на позицию ниже
//
&НаКлиенте
Процедура ПереместитьПозициюСпецификацииВниз(Команда)
	
	КоличествоСтрокСпецификации = Спецификация.Количество();
	
	ТекущийНомерПозицииСпецификации = Элементы.Спецификация.ТекущиеДанные.НомерПозиции;
	
	Если ТекущийНомерПозицииСпецификации = КоличествоСтрокСпецификации Тогда // Ниже некуда
	
		Возврат;	
	
	КонецЕсли;
	
	ИзменитьПорядокСоставляющихЧастейСпецификации(ТекущийНомерПозицииСпецификации, "Вниз");	
	
КонецПроцедуры

// Процедура предназначена для изменения номеров позиций составляющих частей спецификации и сортировки спецификации 
//
&НаКлиенте
Процедура ИзменитьПорядокСоставляющихЧастейСпецификации(ТекущийНомерПозиции, Направление)

	Для каждого Строка Из Спецификация Цикл
		
		Если Направление = "Вверх" Тогда
		
			Если Строка.НомерПозиции = ТекущийНомерПозиции - 1 Тогда
				
				Строка.НомерПозиции = Строка.НомерПозиции + 1;
				
				СпецификацияПриИзменении(, Строка.НомерПозиции);
				
			ИначеЕсли Строка.НомерПозиции = ТекущийНомерПозиции Тогда
		...
		ИначеЕсли Направление = "Вниз" Тогда
			...
			Если Строка.НомерПозиции = ТекущийНомерПозиции Тогда
				
				Строка.НомерПозиции = Строка.НомерПозиции + 1;
				
				СпецификацияПриИзменении(, Строка.НомерПозиции);
				
			ИначеЕсли Строка.НомерПозиции = ТекущийНомерПозиции + 1 Тогда
	...
	Спецификация.Сортировать("НомерПозиции");	

КонецПроцедуры // ИзменитьПорядокСоставляющихЧастейСпецификации()

//процедура выполняет сортировку по текущей колонке таблице спецификации 
&НаКлиенте
Процедура СпецификацияСортировать(Команда)
	
	ТекущаяКолонка = Элементы.Спецификация.ТекущийЭлемент;
	
	Если Не ТекущаяКолонка = Неопределено Тогда
		
		ИмяКолонки = СтрЗаменить(ПолучитьПутьКДанным(ТекущаяКолонка.Имя), "Спецификация.", "");
		
		Если СтрНайти(ИмяКолонки, ".") > 0 Тогда
			
			Сообщить("Сортировка по текущей колонке невозможна!");
			
		Иначе
		
			Спецификация.Сортировать(ИмяКолонки);
...
//процедура выполняет поиск по таблице состава
&НаКлиенте
Процедура СпецификацияНайти(Команда)
	
	Если Не ЗначениеЗаполнено(СпецификацияПоиск) Тогда
	...
	СпецификацияУстановитьОтбор();
		
КонецПроцедуры

//процедура устанавливает/снимает отбор по таблице состава
&НаСервере
Процедура СпецификацияУстановитьОтбор(СнятьОтбор = Ложь)
	...
	Иначе
		...
		ЭлементУсловногоОформления.Отбор.Элементы[0].ПравоеЗначение = ПолучитьРезультатПоиска();
		
	КонецЕсли;
	
КонецПроцедуры

//функция возвращает результат поиска по таблице состава
&НаСервере
Функция ПолучитьРезультатПоиска()
	...
	"ВЫБРАТЬ
	|	ТаблицаСостава.СоставляющаяЧасть КАК СоставляющаяЧасть,
	|	ТаблицаСостава.Обозначение КАК Обозначение,
	|	ТаблицаСостава.Тип КАК Тип
	|ПОМЕСТИТЬ втТаблицаСостава
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втТаблицаСостава.СоставляющаяЧасть КАК СоставляющаяЧасть
	|ИЗ
	|	втТаблицаСостава КАК втТаблицаСостава
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КаталогПредметовСнабжения КАК КаталогПредметовСнабжения
	|		ПО втТаблицаСостава.СоставляющаяЧасть = КаталогПредметовСнабжения.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РазделыСпецификацииЗИП КАК РазделыСпецификацииЗИП
	|		ПО втТаблицаСостава.Тип = РазделыСпецификацииЗИП.Ссылка
	|ГДЕ
	|	(КаталогПредметовСнабжения.Наименование ПОДОБНО &СтрокаПоиска
	|			ИЛИ втТаблицаСостава.Обозначение ПОДОБНО &СтрокаПоиска
	|			ИЛИ ЕСТЬNULL(РазделыСпецификацииЗИП.Наименование, """") ПОДОБНО &СтрокаПоиска)";
	#КонецОбласти
	Запрос.УстановитьПараметр("ТаблицаСостава", РеквизитФормыВЗначение("Спецификация"));
	Запрос.УстановитьПараметр("СтрокаПоиска",	"%" + СпецификацияПоиск + "%");
	
	Результат.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("СоставляющаяЧасть"));
	
	Возврат Результат;
	
КонецФункции

//процедура отменяет поиск по таблице состава
&НаКлиенте
Процедура СпецификацияОтменитьПоиск(Команда)
	
	СпецификацияПоиск 							   = "";
	Элементы.СпецификацияПронумеровать.Доступность = Ложь;
	
	СпецификацияУстановитьОтбор(Истина);
	
КонецПроцедуры

&НаСервере
Функция ПолучитьПутьКДанным(ИмяКолонки)
	
	Возврат Элементы[ИмяКолонки].ПутьКДанным;
	
КонецФункции

&НаКлиенте
Процедура СпецификацияНомерПозицииПриИзменении(Элемент)
	
	// Определим изменение
	
	СтарыйНомерПозиции = ТекущийНомерПозицииСпецификации;
	НовыйНомерПозиции = Элементы.Спецификация.ТекущиеДанные.НомерПозиции;
	
	// При необходимости скорректируем новый номер
	
	Если НовыйНомерПозиции < 1 Тогда
	...
	Если НовыйНомерПозиции > КоличествоСтрокСпецификации Тогда
	
		НовыйНомерПозиции = КоличествоСтрокСпецификации;	
	
	КонецЕсли;
	
	// Определим направление
	
	Если НовыйНомерПозиции = СтарыйНомерПозиции Тогда
	
		Возврат;	
	
	ИначеЕсли НовыйНомерПозиции < СтарыйНомерПозиции Тогда
	...
	// Внесем изменения в спецификацию
	
	ТекущийИндекс = Спецификация.Индекс(Элементы.Спецификация.ТекущиеДанные);
	СпецификацияПриИзменении(, НовыйНомерПозиции);
	
	Для каждого Строка Из Спецификация Цикл
		
		Если Строка.НомерПозиции = НовыйНомерПозиции Тогда
			
			ИндексСтроки = Спецификация.Индекс(Строка);
			
			Если Не ИндексСтроки = ТекущийИндекс Тогда
				
				Строка.НомерПозиции = Строка.НомерПозиции + ?(Направление = "Вверх", 1, -1);
				
				СпецификацияПриИзменении(, Строка.НомерПозиции);
				
			КонецЕсли;
			
		ИначеЕсли Строка.НомерПозиции > НовыйНомерПозиции И Направление = "Вверх" Тогда
			
			Строка.НомерПозиции = Строка.НомерПозиции + 1;
			
			СпецификацияПриИзменении(, Строка.НомерПозиции);
			
		ИначеЕсли Строка.НомерПозиции < НовыйНомерПозиции И Направление = "Вниз" Тогда
	...
	ТекущийНомерПозицииСпецификации = НовыйНомерПозиции;
	
КонецПроцедуры

&НаКлиенте
Процедура ОтложенноеВыполнениеНастроитьПрименяемостьСоставнойЧастиИзделия()
	...
	НастроитьПрименяемостьСоставнойЧастиИзделия(ТекущиеДанные.СоставляющаяЧасть);
...
Процедура СпецификацияПриАктивизацииСтроки(Элемент)
	...
	ОтключитьОбработчикОжидания("ОтложенноеВыполнениеНастроитьПрименяемостьСоставнойЧастиИзделия");
	...
	ТекущийНомерПозицииСпецификации = ТекущиеДанные.НомерПозиции;
	
	ПодключитьОбработчикОжидания("ОтложенноеВыполнениеНастроитьПрименяемостьСоставнойЧастиИзделия",0.5,Истина)
...
Процедура СпецификацияПередУдалением(Элемент, Отказ)
	...
	НомерПозицииУдаленнойСтрокиСпецификации = ТекущиеДанные.НомерПозиции;	
...
Процедура СпецификацияПослеУдаления(Элемент)
	...
	Если Не НомерПозицииУдаленнойСтрокиСпецификации > Спецификация.Количество() Тогда
	
		Для каждого Строка Из Спецификация Цикл
		
			Если Строка.НомерПозиции > НомерПозицииУдаленнойСтрокиСпецификации Тогда
...
&НаКлиенте
Процедура ПоказатьИсториюСпецификации(Команда)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ПредметСнабжения", Объект.Ссылка);
	
	ОткрытьФорму("Справочник.КаталогПредметовСнабжения.Форма.ФормаПросмотрИсторииСпецификации", ПараметрыОткрытия, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца); 
...
// Процедура предназначена для изменения порядковых номеров строк таблицы ресурсов и сроков службы путем перемещения одной строки на позицию выше
//
&НаКлиенте
...
// Процедура предназначена для изменения порядковых номеров строк таблицы ресурсов и сроков службы путем перемещения одной строки на позицию ниже
//
&НаКлиенте
Процедура ПереместитьСтрокуРесурсовИСроковСлужбыВниз(Команда)
	
	КоличествоСтрокРесурсовИСроковСлужбы = Спецификация.Количество();
...
// Процедура предназначена для изменения порядковых номеров строк таблицы регламента ТО путем перемещения одной строки на позицию выше
//
&НаКлиенте
...
// Процедура предназначена для изменения порядковых номеров строк таблицы регламента ТО путем перемещения одной строки на позицию ниже
//
&НаКлиенте
...
// Процедура предназначена для изменения порядковых номеров строк таблицы ЗИП для ТО путем перемещения одной строки на позицию выше
//
&НаКлиенте
...
// Процедура предназначена для изменения порядковых номеров строк таблицы ЗИП для ТО путем перемещения одной строки на позицию ниже
//
&НаКлиенте
...
&НаКлиенте
Процедура ПоказатьВсюСтруктуру(Команда)
	...
	МассивСтруктурСоставныхЧастей = Новый Массив;
	
	Для каждого Строка Из Спецификация Цикл
	
		Если ЗначениеЗаполнено(Строка.СоставляющаяЧасть) Тогда
			
			СтруктураСоставнойЧасти = Новый Структура();
			СтруктураСоставнойЧасти.Вставить("СоставнаяЧасть", 		Строка.СоставляющаяЧасть);
			СтруктураСоставнойЧасти.Вставить("ЗИП", 				Строка.ЗИП);
			СтруктураСоставнойЧасти.Вставить("Тип", 				Строка.Тип);
			СтруктураСоставнойЧасти.Вставить("Количество", 			Строка.Количество);
			СтруктураСоставнойЧасти.Вставить("ПричинаОтбораДляЗИП", Строка.ПричинаВыбораДляЗИП);
			СтруктураСоставнойЧасти.Вставить("Взаимозаменяемость", 	Строка.Взаимозаменяемость);
		
			МассивСтруктурСоставныхЧастей.Добавить(СтруктураСоставнойЧасти);	
	...
	ПараметрыОткрытия = Новый Структура();
	ПараметрыОткрытия.Вставить("ПредметСнабжения", Объект.Ссылка);
	ПараметрыОткрытия.Вставить("МассивСтруктурСоставныхЧастей", МассивСтруктурСоставныхЧастей);
...
&НаКлиенте
Процедура АналогиАналогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ТекФорма = ПолучитьФорму("Справочник.КаталогПредметовСнабжения.ФормаВыбора",,Элемент);
...
//+ 22.11.2017 15:04:17 Базунов Д.А. Задача: 
&НаКлиенте
Процедура ИзготовителиИПоставщикиКонтрагентПриИзменении(Элемент)
	ОбновитьГородВПоставщиках();
КонецПроцедуры

&НаКлиенте
Процедура ИзготовителиИПоставщикиЕдиницаИзмеренияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработчикВыбораЕдиницыИзмерения", ЭтотОбъект);
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ВыборДляПредметовСнабжения");
	
	ОткрытьФорму("Справочник.ОКЕИ.Форма.ФормаВыбора", ПараметрыОткрытия, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца); 
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработчикВыбораЕдиницыИзмерения(ЕдиницаИзмерения, ДопПараметры = Неопределено) Экспорт

	Если Не ЗначениеЗаполнено(ЕдиницаИзмерения) Тогда
		Возврат;
	КонецЕсли;	
	
	ТекущиеДанные = Элементы.ИзготовителиИПоставщики.ТекущиеДанные;
...
//++ 30.11.2017 Веденеев П. //перенумерация элементов спецификации
&НаКлиенте
Процедура Пронумеровать(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ПронумероватьЗавершение", ЭтаФорма), "Нумерация элементов будет изменена. Продолжить?", РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура ПронумероватьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		
		Состав = Спецификация;
		ВыполнитьПеренумерацию(Состав);
		КопироватьДанныеФормы(Состав, Спецификация);
		
		МодифицированностьСпецификации = Истина;
...
&НаСервереБезКонтекста
Процедура ВыполнитьПеренумерацию(СоставПС)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СоставПС.СоставляющаяЧасть КАК СоставляющаяЧасть,
	|	СоставПС.Тип КАК Тип,
	|	СоставПС.Количество КАК Количество,
	|	СоставПС.ОбозначениеСНК КАК ОбозначениеСНК,
	|	СоставПС.ПричинаВыбораДляЗИП КАК ПричинаВыбораДляЗИП,
	|	СоставПС.УказательНаЭлементСхемы КАК УказательНаЭлементСхемы,
	|	СоставПС.Взаимозаменяемость КАК Взаимозаменяемость,
	|	СоставПС.ПризнакПодбора КАК ПризнакПодбора,
	|	СоставПС.ОбозначениеЗаготовки КАК ОбозначениеЗаготовки,
	|	СоставПС.НомерПозиции КАК НомерПозиции,
	|	СоставПС.СведенияОбИзменениях КАК СведенияОбИзменениях,
	|	СоставПС.ЗИП КАК ЗИП
	|ПОМЕСТИТЬ втСпецификация
	|ИЗ
	|	&СоставПС КАК СоставПС
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	втСпецификация.Количество КАК Количество,
	|	втСпецификация.ОбозначениеСНК КАК ОбозначениеСНК,
	|	втСпецификация.ПричинаВыбораДляЗИП КАК ПричинаВыбораДляЗИП,
	|	втСпецификация.УказательНаЭлементСхемы КАК УказательНаЭлементСхемы,
	|	втСпецификация.Взаимозаменяемость КАК Взаимозаменяемость,
	|	втСпецификация.ПризнакПодбора КАК ПризнакПодбора,
	|	втСпецификация.ОбозначениеЗаготовки КАК ОбозначениеЗаготовки,
	|	ВЫБОР
	|		КОГДА втСпецификация.НомерПозиции = 0
	|			ТОГДА 999999
	|		ИНАЧЕ втСпецификация.НомерПозиции
	|	КОНЕЦ КАК НомерПозиции,
	|	втСпецификация.СведенияОбИзменениях КАК СведенияОбИзменениях,
	|	втСпецификация.ЗИП КАК ЗИП,
	|	КаталогПредметовСнабжения.Обозначение КАК Обозначение
	|ИЗ
	|	втСпецификация КАК втСпецификация
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КаталогПредметовСнабжения КАК КаталогПредметовСнабжения
	|		ПО втСпецификация.СоставляющаяЧасть = КаталогПредметовСнабжения.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерПозиции,
	|	КаталогПредметовСнабжения.Наименование";
	Запрос.УстановитьПараметр("СоставПС", СоставПС.Выгрузить());
	...
	СоставПС.Загрузить(ТаблицаЗапроса);
	
КонецПроцедуры
//-- 30.11.2017 Веденеев П. //перенумерация элементов спецификации

// + 22.01.2018 10:31:51 Базунов Д.А. Задача: 
...
//++ 25.01.2018 Веденеев П. //добавлено для корректной реализации ОПЗ 5
...
// ++ 21.05.2018 11:39:12 Базунов Д.А. Задача: 
&НаСервере
...
//++ 27.06.2018 Веденеев П. запрет на добавление одинаковых элементов в табличные части
...
Процедура ХарактеристикиХарактеристикаПриИзменении(Элемент)
	...
	ПроверкаНаличияЭлементаВТаблице("Характеристики", "Характеристика");
...
Процедура ХарактеристикиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование Тогда
		...
		Сообщить("Нельзя добавлять характеристики копированием!");
		Отказ = Истина;
...
&НаКлиенте
Процедура ИзготовителиИПоставщикиПриИзменении(Элемент)
	
	ПроверкаНаличияЭлементаВТаблице("ИзготовителиИПоставщики", "Контрагент");
		
КонецПроцедуры

&НаКлиенте
Процедура ИзготовителиИПоставщикиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	
	Если Копирование Тогда
		
		Сообщить("Нельзя добавлять поставщиков копированием!");
		Отказ = Истина;
...
&НаКлиенте
Процедура СпецификацияПоискПриИзменении(Элемент)
	СпецификацияНайти(Неопределено);
КонецПроцедуры // СпецификацияПоискПриИзменении

//-- 27.06.2018 Веденеев П. запрет на добавление одинаковых характеристик
...
Процедура ОбновитьДинамическиеСпискиТабличныхЧастей()
	...
	Элементы.ИзготовителиИПоставщики.Обновить();
	
	СпецификацияСлужебная();
	СпецификацияИзменена = СпецификацияИзменена();
...
&НаКлиенте
Процедура СпецификацияДинамическийСписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
...
Процедура СпецификацияДинамическийСписокПередНачаломДобавленияДалее(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		//ПараметрыОткрытия = Новый Структура;
		ОткрытьФорму("РегистрСведений.СпецификацииПредметовСнабжения.Форма.ФормаЗаписи", ПараметрыОткрытия, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПеренумероватьСпецификацию(ПредметСнабжения)
	...
	НаборЗаписей = РегистрыСведений.СпецификацииПредметовСнабжения.СоздатьНаборЗаписей();
...
&НаСервере
Процедура СпецификацияСлужебная()
	...
	СпецификацияСлужебная.Загрузить(РезультатЗапроса.Выгрузить());
	
КонецПроцедуры // СпецификацияСлужебная()

&НаСервере
Функция СпецификацияИзменена()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СпецификацииПредметовСнабженияСлужебная.СоставляющаяЧасть КАК СоставляющаяЧасть,
	|	СпецификацииПредметовСнабженияСлужебная.ЗИП КАК ЗИП
	|ПОМЕСТИТЬ ВТ_СпецификацииПредметовСнабженияСлужебная
	|ИЗ
	|	&СпецификацияСлужебная КАК СпецификацииПредметовСнабженияСлужебная
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СпецификацииПредметовСнабженияСлужебная.ЗИП КАК ЗИП
	|ИЗ
	|	ВТ_СпецификацииПредметовСнабженияСлужебная КАК СпецификацииПредметовСнабженияСлужебная
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	СпецификацииПредметовСнабжения.ЗИП
	|ИЗ
	|	РегистрСведений.СпецификацииПредметовСнабжения КАК СпецификацииПредметовСнабжения
	|ГДЕ
	|	СпецификацииПредметовСнабжения.ПредметСнабжения = &ПредметСнабжения";
	...
	Запрос.УстановитьПараметр("ПредметСнабжения", Объект.Ссылка);
	Запрос.УстановитьПараметр("СпецификацияСлужебная", СпецификацияСлужебная.Выгрузить());
	
	РезультатЗапроса = Запрос.Выполнить().Выгрузить();
	РезультатЗапроса.Свернуть("СоставляющаяЧасть,ЗИП", "Счетчик");
	
	Возврат РезультатЗапроса.Найти(1,"Счетчик") <> Неопределено;
	
КонецФункции

&НаСервереБезКонтекста
Процедура УдалитьСоставляющуюЧастьИзСпецификацииНаСервере(ПредметСнабжения, СоставляющаяЧасть, ЗИП)
	
	НаборЗаписей = РегистрыСведений.СпецификацииПредметовСнабжения.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ПредметСнабжения.Установить(ПредметСнабжения);
	НаборЗаписей.Отбор.СоставляющаяЧасть.Установить(СоставляющаяЧасть);
	НаборЗаписей.Отбор.ЗИП.Установить(ЗИП);
	НаборЗаписей.Записать();
...
Процедура УдалитьСоставляющуюЧастьИзСпецификации(Команда)
	...
	Если Элементы.СпецификацияДинамическийСписок.ТекущиеДанные <> Неопределено Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Вы действительно хотите удалить составляющую часть из спецификации?';"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура УдалитьСоставляющуюЧастьИзСпецификацииДалее(Результат, ДополнительныеПараметры) Экспорт
	...
	УдалитьСоставляющуюЧастьИзСпецификацииНаСервере(Объект.Ссылка, ТекущаяСтрока.СоставляющаяЧасть, ТекущаяСтрока.ЗИП);
	Элементы.СпецификацияДинамическийСписок.Обновить();
	СпецификацияИзменена = СпецификацияИзменена();
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если СпецификацияИзменена Тогда
		Если НЕ ЗавершениеРаботы = Истина Тогда
			ДобавитьПредметСнабжениявОчередьИзмененияСпецификации();
		Иначе
			Отказ = Истина;
...
&НаСервере
Процедура ДобавитьПредметСнабжениявОчередьИзмененияСпецификации()
	
	РезультатЗаписиВРегистр = СтруктураЗаказаСервер.ДобавитьПредметСнабжениявОчередьИзмененияСпецификации(Объект.Ссылка);
