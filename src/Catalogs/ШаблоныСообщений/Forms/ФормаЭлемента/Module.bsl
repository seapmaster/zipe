...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ВариантыНазначения = 1 И ПустаяСтрока(Объект.ПолноеИмяТипаПараметраВводаНаОсновании) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите назначение шаблона сообщения'"),, "Объект.ПолноеИмяТипаПараметраВводаНаОсновании");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	// + 30.01.2018 14:59:45 Базунов Д.А. Задача: 
	// Закоментировал код, т.к. есть проверка по кнопке на форме
	...
	#Если Не ВебКлиент Тогда
		ПоместитьНовыеФайлыВоВременноеХранилищеКлиент(Вложения, УникальныйИдентификатор, Отказ);
	#КонецЕсли
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПоместитьНовыеФайлыВоВременноеХранилищеСервер(Вложения, УникальныйИдентификатор, Отказ);
...
Процедура ВариантыНазначенияДляВсехПриИзменении(Элемент)
	...
	Элементы.ПолноеИмяТипаПараметраВводаНаОсновании.АвтоОтметкаНезаполненного = (ВариантыНазначения = 1);
...
Процедура УстановитьВыборФормата(Знач ФорматыСохранения = Неопределено)
	...
	// + 22.01.2018 15:36:19 Базунов Д.А. Задача: Было: Не ЕстьВыбранныйФормат
	Если ЕстьВыбранныйФормат Тогда
		ВыбранныеФорматыСохранения[0].Пометка = Истина;
	КонецЕсли;
	// - 22.01.2018 15:36:19 Базунов Д.А. Задача:
...
Процедура ПоместитьНовыеФайлыВоВременноеХранилищеСервер(Вложения, УникальныйИдентификатор, Отказ)
...
&НаКлиенте
Асинх Процедура ПоместитьНовыеФайлыВоВременноеХранилищеКлиент(Вложения, УникальныйИдентификатор, Отказ)
	...
	Для Каждого СтрокаТаблицыВложений Из Вложения Цикл
		Если СтрокаТаблицыВложений.Статус = "ВнешнийНовый" Тогда
			Попытка
				ЗагружаемыйФайл = Новый Файл(СтрокаТаблицыВложений.Имя);
				Если Ждать ЗагружаемыйФайл.СуществуетАсинх() Тогда
					Данные = Новый ДвоичныеДанные(СтрокаТаблицыВложений.Имя);
					СтрокаТаблицыВложений.Имя = ПоместитьВоВременноеХранилище(Данные, УникальныйИдентификатор);
				КонецЕсли;
			Исключение
