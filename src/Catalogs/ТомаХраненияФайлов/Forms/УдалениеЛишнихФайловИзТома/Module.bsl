...
&НаКлиенте
Асинх Процедура УдалитьЛишниеФайлы(Команда)
	...
	Если Не Ждать ПравоЗаписиВКаталог() Тогда
		Возврат;
	КонецЕсли;
		...
		Попытка
			Если Не ПустаяСтрока(ПутьКаталогаДляКопирования) Тогда
				ИмяФайлаСПутем = ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьУникальноеИмяСПутем(
					...
					ЛишнийФайл.Имя);
				Ждать ПереместитьФайлАсинх(ЛишнийФайл.ПолноеИмя, КаталогДляКопирования + ИмяФайлаСПутем);
			Иначе
				Ждать УдалитьФайлыАсинх(ЛишнийФайл.ПолноеИмя);
			КонецЕсли;
...
&НаКлиенте
Асинх Функция ПравоЗаписиВКаталог()
	...
	Если Не ПустаяСтрока(ПутьКаталогаДляКопирования) Тогда
		...
		Попытка
			Ждать СоздатьКаталогАсинх(ИмяКаталогаТестовое);
			Ждать УдалитьФайлыАсинх(ИмяКаталогаТестовое);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
