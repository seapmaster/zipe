...
Асинх Процедура ПриОткрытии(Отказ)
	...
	Если ЕстьПравоДобавления Тогда
		ДоступнаКомандаСканировать = Ждать ФайловыеФункцииСлужебныйКлиент.ДоступнаКомандаСканировать();
...
&НаКлиенте
Асинх Процедура СписокПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Файл")
	   И Ждать ПараметрыПеретаскивания.Значение.ЭтоФайлАсинх() Тогда
		...
		Если ПараметрыПеретаскивания.Значение.Количество() >= 1
		   И ТипЗнч(ПараметрыПеретаскивания.Значение[0]) = Тип("Файл") Тогда
			...
			Для каждого Значение Из ПараметрыПеретаскивания.Значение Цикл
				...
				Если ТипЗнч(Значение) = Тип("Файл") И Ждать Значение.ЭтоФайлАсинх() Тогда
