...
Процедура ВыгрузитьВФайл(ПутьКФайлу, Каталог = "")
	...
	Состояние("Начало выгрузки", 0, "Выгрузка в файл");
	...
	Исключение 
		Сообщить("Ошибка при запуске приложения Excel"+ОписаниеОшибки());
	КонецПопытки;	
	...
	//Сделаем шапку, из синонимов реквизитов ТЧ
	Для каждого Рекв Из КолонкиМетаданные Цикл
		Лист.Cells(СчетчикСтрок, СчетчикКолонок).Value = Рекв.Синоним;
	...
	Для каждого ТекСтрока Из Объект.ПредметыСнабжения Цикл
		...
		Для каждого ТекКолонка Из КолонкиМетаданные Цикл
			...
			Лист.Cells(СчетчикСтрок, СчетчикКолонок).Value = Строка(ТекСтрока[ТекКолонка.Имя]);
		...
		Для каждого ТекФайл Из МассивФайлов Цикл
			...
			ДанныеФайла = ПрисоединенныеФайлыКлиент.ПолучитьДанныеФайла(ТекФайл);
			// Сохраняем
		...
		Состояние("Выгружено:", СчетчикСтрок-1, "Выгрузка в файл");
	...
	Состояние("Выгрузка завершена", 100, "Выгрузка в файл");
