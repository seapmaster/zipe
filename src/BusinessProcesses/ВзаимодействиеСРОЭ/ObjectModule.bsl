...
Процедура ВариантыДальнейшихДействийОбработкаВыбораВарианта(ТочкаВыбораВарианта, Результат)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗадачаИсполнителя.ВариантВыбораДляЗагрузки КАК ВариантВыбораДляЗагрузки
	|ИЗ
	...
	Запрос.УстановитьПараметр("ТочкаМаршрута", БизнесПроцессы.ВзаимодействиеСРОЭ.ТочкиМаршрута.ОжиданиеПолученияОтвета);
	Запрос.УстановитьПараметр("БизнесПроцесс", Ссылка);
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ВариантВыбораДляЗагрузки = 1 Тогда
			Результат = ТочкаВыбораВарианта.Варианты.ДанныеПолучены;
		ИначеЕсли Выборка.ВариантВыбораДляЗагрузки = 2 Тогда
			Результат = ТочкаВыбораВарианта.Варианты.Отклонено;
		КонецЕсли; 
