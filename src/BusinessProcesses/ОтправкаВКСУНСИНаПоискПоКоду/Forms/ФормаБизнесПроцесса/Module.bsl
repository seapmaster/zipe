...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Объект.ВариантыПоискаПоКоду = Перечисления.ВариантыПоискаПоКоду.ФНН;
...
&НаСервере
Функция ПолучитьРеквизитПС(ТекущийПС)
	
	Возврат ТекущийПС[Строка(Объект.ВариантыПоискаПоКоду)];
...
Процедура ПредметыСнабженияПриИзменении(Элемент)
	...
	Элемент.ТекущиеДанные.ОпорныйКод = ПолучитьРеквизитПС(Элемент.ТекущиеДанные.ПредметСнабжения);
	
КонецПроцедуры

&НаСервере
Процедура ВариантыПоискаПоКодуПриИзмененииНаСервере()
	
	Для каждого Стр Из Объект.ПредметыСнабжения Цикл
		Стр.ОпорныйКод = Стр.ПредметСнабжения[Строка(Объект.ВариантыПоискаПоКоду)];
	КонецЦикла; 
	
КонецПроцедуры

&НаКлиенте
Процедура ВариантыПоискаПоКодуПриИзменении(Элемент)
	
	ВариантыПоискаПоКодуПриИзмененииНаСервере();
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ТекРеквизит = Строка(Объект.ВариантыПоискаПоКоду);
	ЕстьОшибки = Ложь;
	
	Для каждого Элемент Из Объект.ПредметыСнабжения Цикл
		Если НЕ ЗначениеЗаполнено(СокрЛП(Элемент.ОпорныйКод)) Тогда
			Сообщить("Не заполнен """+ТекРеквизит+""" у " + Строка(Элемент.ПредметСнабжения)+" (Номер строки: "+Элемент.НомерСтроки+")");
			ЕстьОшибки = Истина;
		КонецЕсли; 
	КонецЦикла; 
