...
&НаКлиенте
Процедура ПредметыСнабженияВыборЗавершение1(ИмяКаталогаВременныхФайлов, ДополнительныеПараметры) Экспорт
...
КонецПроцедуры // ПредметыСнабженияВыборЗавершение
...
&НаКлиенте
Асинх Процедура ВыгрузитьВФайлИСтартовать(Команда)
	...
	Иначе
		НачатьУдалениеФайлов(Новый ОписаниеОповещения("ВыгрузитьВФайлИСтартоватьЗавершение", ЭтотОбъект), ПутьКФайлу);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьВФайлИСтартоватьЗавершение(ДополнительныеПараметры) Экспорт
	Сообщить("Не удалось стартовать бизнес-процесс. Повторите попытку");
КонецПроцедуры // ВыгрузитьВФайлИСтартоватьЗавершение
