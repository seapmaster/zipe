...
Процедура ВыполнитьПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	...
	Для каждого Задача Из ФормируемыеЗадачи Цикл
		...
		ИначеЕсли ТочкаМаршрутаБизнесПроцесса = ТочкиМаршрута.ПринятиеРешенияОбИзменении Тогда
		...
		Если ТочкаМаршрутаБизнесПроцесса = ТочкиМаршрута.ИсправлениеЗамечанийПоСтруктуреКорабля Тогда
			
			ПричинаДоработки = ПолучитьЗначениеРеквизитаЗадачи(ТочкиМаршрута.ПринятиеРешенияОбИзменении, "РезультатВыполнения", Задача.Ссылка);  
			
			Если ЗначениеЗаполнено(ПричинаДоработки) Тогда
				
				Задача.Описание = КартаБП[ТочкаМаршрутаБизнесПроцесса.Имя].Подсказка + Символы.ПС + ПричинаДоработки;
				
			КонецЕсли;
			
			АвторКорректировки = ПолучитьЗначениеРеквизитаЗадачи(ТочкиМаршрута.РедактированиеСтруктурыКорабля, "Исполнитель", Задача.Ссылка);
			
			Если ЗначениеЗаполнено(АвторКорректировки) Тогда
				
				Задача.Исполнитель = АвторКорректировки;
...
Функция ПолучитьЗначениеРеквизитаЗадачи(ТочкаМаршрута, ИмяРеквизита, Задача)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗадачаИсполнителя.Номер КАК ЗначениеРеквизита
	|ИЗ
...
Процедура РешениеОбИзмененииОбработкаВыбораВарианта(ТочкаВыбораВарианта, Результат)
	...
	|	И ЗадачаИсполнителя.ТочкаМаршрута = &ТочкиМаршрута
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЗадачаИсполнителя.Дата УБЫВ";
	
	Запрос.УстановитьПараметр("БизнесПроцесс", Ссылка);
	Запрос.УстановитьПараметр("ТочкиМаршрута", БизнесПроцессы.РедактированиеЭлектроннойСтруктурыКорабля.ТочкиМаршрута.ПринятиеРешенияОбИзменении);
