...
Процедура ВариантЗагрузкиЗаявкиОбработкаВыбораВарианта(ТочкаВыбораВарианта, Результат)
	
	ТипыВходящихЗапросов = Перечисления.ТипыВходящихЗапросов;
	
	Если ТипЗапроса = ТипыВходящихЗапросов.ДобавлениеФайлов Тогда
		
		Результат = ТочкаВыбораВарианта.Варианты.ДобавлениеФайлов;
		
	ИначеЕсли ТипЗапроса = ТипыВходящихЗапросов.ЗагрузкаЗаявок Тогда
		
		Результат = ТочкаВыбораВарианта.Варианты.НоваяЗаявка;
		
	ИначеЕсли ТипЗапроса = ТипыВходящихЗапросов.ИзменениеЗаявки Тогда
		
		Результат = ТочкаВыбораВарианта.Варианты.ИзменениеЗаявки;
		
	КонецЕсли; 
	
КонецПроцедуры

Процедура ФормированиеЗаявокПередСозданиемЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, СтандартнаяОбработка)
	
	Если НЕ ТочкаМаршрутаБизнесПроцесса = БизнесПроцессы.УведомлениеОФормированииЗаявки.ТочкиМаршрута.ФормированиеЗаявок Тогда
		Возврат;
	КонецЕсли; 
	...
	Пока Выборка.Следующий() Цикл
		...
		НаименованиеДляЗадачи = "Формирование заявки "+Заявка.Номер + " от " + Формат(Заявка.Дата, "ДФ=dd.MM.yyyy");
		
		// Создаем и записываем ведущую задачу      
		НоваяЗадача = Задачи.ЗадачаИсполнителя.СоздатьЗадачу();
		НоваяЗадача.БизнесПроцесс = Ссылка;
		НоваяЗадача.ТочкаМаршрута = БизнесПроцессы.УведомлениеОФормированииЗаявки.ТочкиМаршрута.ФормированиеЗаявок;
		НоваяЗадача.Дата = ТекущаяДата();
		НоваяЗадача.Наименование = НаименованиеДляЗадачи;
		...
		НоваяЗадача.Предмет = Заявка;
		//НоваяЗадача.Исполнитель = Выборка.РуководительПроектаВТС;
		НоваяЗадача.Записать();
		
		ФормируемыеЗадачи.Добавить(НоваяЗадача.Ссылка);
		
		НовыйБП = БизнесПроцессы.ФормированиеЗаявки.СоздатьБизнесПроцесс();
		
		НовыйБП.Дата		 = ТекущаяДата();
		НовыйБП.Наименование = НаименованиеДляЗадачи;
		НовыйБП.Исполнитель  = Выборка.РуководительПроектаВТС;
		НовыйБП.Предмет 	 = Заявка;
		НовыйБП.ВедущаяЗадача = НоваяЗадача.Ссылка;
