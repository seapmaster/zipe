...
Процедура ПриОпределенииСправочниковДляЗагрузкиДанных(ЗагружаемыеСправочники) Экспорт
	...
	СтрокаТаблицы = ЗагружаемыеСправочники.Найти(Метаданные.Справочники.ОКСМ.ПолноеИмя(), "ПолноеИмя");
...
Функция СведенияОбАдресеВВидеСтруктуры(Адрес, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Формат9Запятых Тогда
		...
		Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
			...
			Если ПустаяСтрока(Состав.Страна) Тогда
				Страна = Справочники.ОКСМ.Россия;
				НаименованиеСтраны = Страна.Наименование;
				КодСтраны = СокрЛП(Страна.Код);
			Иначе
				Страна = Справочники.ОКСМ.НайтиПоНаименованию(Состав.Страна, Истина);
				НаименованиеСтраны = Страна.Наименование;
				КодСтраны = СокрЛП(Страна.Код);
			КонецЕсли;
	...
	ЭтоРоссийскийАдрес = ?(СтрСравнить(НаименованиеСтраны, Справочники.ОКСМ.Россия.Наименование) = 0, Истина, Ложь);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Справочники.ВидыКонтактнойИнформации.ОбновитьВидПоляДругое";
	Обработчик.РежимВыполнения = "Отложенно";
...
Процедура ПодготовкаЭталонаРазделенныхСтранМира() Экспорт
	...
	Запрос = Новый Запрос("
		...
		|	Справочник.БуквенныйКодАльфа2        КАК КодАльфа2,
		|	Справочник.БуквенныйКодАльфа3        КАК КодАльфа3, 
		|	Справочник.ПолноеНаименованиеСтраны  КАК НаименованиеПолное
		|ИЗ
		|	Справочник.ОКСМ КАК Справочник
		|");
...
Процедура ОбновлениеПоЭталонуРазделенныхСтранМира() Экспорт
	...
	Запрос = Новый Запрос("
		...
		|	Эталон КАК Эталон
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	Справочник.ОКСМ КАК СтраныМира
		|ПО
...
Процедура ОбновитьСуществующиеСтраныМира() Экспорт
	...
	Для Каждого СтрокаКлассификатора Из Справочники.ОКСМ.ТаблицаКлассификатора() Цикл
...
Функция СформироватьАдресПоПредставлению(Представление)
	...
	НаименованиеРоссии = СокрЛП(Справочники.ОКСМ.Россия.Наименование);
	...
	Иначе
		...
		ДанныеСтраныМира = Справочники.ОКСМ.ДанныеОКСМ(, Адрес.Страна);
		Если ДанныеСтраныМира <> Неопределено И НЕ ЗначениеЗаполнено(ДанныеСтраныМира.Ссылка) Тогда
			СтранаМира = Справочники.ОКСМ.СсылкаПоДаннымКлассификатора(ДанныеСтраныМира);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОпределитьСтрануИИндекс(ДанныеАдреса)
	...
	Классификатор = Справочники.ОКСМ.ТаблицаКлассификатора();
...
Функция НайтиДополнительныйЭлементАдреса(АдресРФ) Экспорт
	...
	Для каждого ВариантДополнительныйЭлементАдреса Из ВариантыДополнительногоЭлементаАдреса Цикл
		XPath = УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(90, ВариантДополнительныйЭлементАдреса);
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	Для Каждого ДанныеПомещения Из ЗданияПомещения.Помещения Цикл
		Если СтрДлина(ДанныеПомещения.Значение) > 12 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, ДанныеПомещения.ПутьXPath,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение поля ""%1"" должно быть короче 12 символов.'"), ДанныеПомещения.Тип), ДанныеПомещения.Тип);
		КонецЕсли;
	КонецЦикла;
...
Функция АдресВСтарыйСписокПолей(XDTOАдрес, РасширенныйСоставПолей = Истина)
	...
	Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		...
		Если ПустаяСтрока(XDTOАдрес.Страна) Тогда
			КодСтраны = "";
		Иначе
			Страна = Справочники.ОКСМ.НайтиПоНаименованию(XDTOАдрес.Страна, Истина);
			КодСтраны = СокрЛП(Страна.Код);
		КонецЕсли;
...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	НазваниеРоссии  = ВРег(Справочники.ОКСМ.Россия.Наименование);
...
Процедура ЗаменитьВСтруктуреНеопределеноНаПустуюСтроку(СтруктураДляОбхода)
