...
Функция ПараметрыАутентификацииНаСайте() Экспорт
	...
	Иначе
		...
		Возврат Неопределено; // Базунов Д.
	КонецЕсли;
...
Процедура ЗагрузитьКлассификаторАдресовССайта(КодыСубъектовРФ = Неопределено, ИсходнаяАвторизация = Неопределено)
	...
	Для Каждого КодСубъекта Из КодыСубъектовРФ Цикл
		...
		// Базунов +
		ЗагруженныйФайл = Новый Структура("Статус", Ложь);
...
Функция ДоступныеВерсииАдресныхСведений(ПараметрыВыгрузки = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	ПараметрыПолучения = Новый Структура("Таймаут", 180);
	
	// Базунов +
	
	РезультатПолученияФайла = Новый Структура("Статус", Ложь);
	//РезультатПолученияФайла = ПолучениеФайловИзИнтернета.СкачатьФайлНаСервере(АдресОписания.Адрес, ПараметрыПолучения);
...
Процедура ЗагрузитьДомаЗданияСтроенияПорциями(КодСубъектаРФ, ФайлЗагрузки)
	...
	Перем ВременнаяТаблица, Запись, Индекс, Индексы, КлючевыеКолонки, Набор, НайденныеСтроки, НоваяСтрока, ОписаниеЗданийXDTO, Отбор, Сжатие, Строка, Таблица, ТекущаяЗапись, ТипXMLСтрока, Фильтр;
...
Процедура ЗагрузитьИсториюАдресныхОбъектовПорциями(КодСубъектаРФ, ФайлЗагрузки)
	...
	Перем ВременнаяТаблица, Запись, Набор, НайденныеСтроки, НоваяСтрока, Отбор, Строка, ТекущаяЗапись, Уровень, Уровни;
