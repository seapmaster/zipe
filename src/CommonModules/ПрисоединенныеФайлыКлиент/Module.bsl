...
//
Процедура ДобавитьФайлы(Знач ВладелецФайла, Знач ИдентификаторФормы, Знач Фильтр = "", ДопПараметры = Неопределено) Экспорт
	...
	// в этом случае не будет открывать диалог выбора файла
	Если Не ДопПараметры = Неопределено И ТипЗнч(ДопПараметры) = Тип("Структура") И ДопПараметры.Свойство("ПолноеИмяФайлаЗагрузка") Тогда
		Параметры.Вставить("ПолноеИмяФайла", ДопПараметры.ПолноеИмяФайлаЗагрузка);
	КонецЕсли; // Если Не ДопПараметры = Неопределено И ТипЗнч(ДопПараметры) = Тип("Структура") И ДопПараметры.Свойство("ПолноеИмяФайлаЗагрузка") Тогда
	
	Параметры.Вставить("ДопПараметры", ДопПараметры); // 22.03.2017 13:45:55 Савинов Александр 
