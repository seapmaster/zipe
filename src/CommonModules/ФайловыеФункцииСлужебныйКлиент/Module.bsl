...
Процедура ВывестиНапоминаниеПриРедактировании(ОбработчикРезультата) Экспорт
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если НЕ РасширениеРаботыСФайламиПодключено() Тогда
			ОткрытьФорму("ОбщаяФорма.НапоминаниеПриРедактировании", , , , , , ОбработчикРезультата);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Асинх Функция РабочийКаталогПользователя() Экспорт
	 
	Возврат Ждать ФайловыеФункцииСлужебныйКлиентПовтИсп.РабочийКаталогПользователя();
...
//
Асинх Функция КаталогВыгрузки() Экспорт
...
#Если Не ВебКлиент Тогда
	...
	Если Путь = Неопределено Тогда
		Если НЕ ПараметрыКлиента.ЭтоБазоваяВерсияКонфигурации Тогда
			Путь = Ждать КаталогМоиДокументы();
...
//
Асинх Функция КаталогМоиДокументы() Экспорт
	Возврат Ждать КаталогДокументовАсинх();
КонецФункции

// Показывает пользователю диалог выбора файлов и возвращает
// массив - выбранные файлы для импорта.
//
Асинх Функция ИмпортируемыеФайлы() Экспорт
	...
	Если НЕ РезультатВыбрать = Неопределено Тогда 
		Для Каждого ИмяФайла Из РезультатВыбрать Цикл
...
Асинх Процедура ОбходФайловРазмер(Путь, МассивФайлов, РазмерСуммарный, КоличествоСуммарное) Экспорт
	
	Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
		
		Если Ждать ВыбранныйФайл.ЭтоКаталогАсинх() Тогда
			НовыйПуть = Строка(Путь);
			
			НовыйПуть = НовыйПуть + ПолучитьРазделительПути();
			
			НовыйПуть = НовыйПуть + Строка(ВыбранныйФайл.Имя);
			МассивФайловВКаталоге = Ждать НайтиФайлыАсинх(НовыйПуть, "*.*");
		...
		РазмерСуммарный = РазмерСуммарный + Ждать ВыбранныйФайл.РазмерАсинх();
...
//
Асинх Функция ВыбратьПутьККаталогуДанныхПользователя() Экспорт
	
	ИмяКаталога = "";
	Если РасширениеРаботыСФайламиПодключено() Тогда
		ИмяКаталога = Ждать РабочийКаталогДанныхПользователяАсинх();
	КонецЕсли;
...
Асинх Функция ОткрытьПроводникСФайлом(Знач ПолноеИмяФайла) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		ЗапуститьПриложениеАсинх("explorer.exe /select, """ + ФайлНаДиске.ПолноеИмя + """");
	Иначе
		ЗапуститьПриложениеАсинх(ФайлНаДиске.Путь);
	КонецЕсли;
...
// 
Асинх Процедура ДействиеПриОткрытииФайлаВРабочемКаталоге(ОбработчикРезультата, ИмяФайлаСПутем, ДанныеФайла) Экспорт
	...
	Параметры.Вставить("ДатаИзмененияУниверсальнаяВРабочемКаталоге",
		Ждать Файл.ПолучитьУниверсальноеВремяИзмененияАсинх());
	...
	Параметры.Вставить("РазмерВРабочемКаталоге", Ждать Файл.РазмерАсинх());
...
//            всегда ложь.
//
Асинх Функция РасширениеРаботыСФайламиПодключено() Экспорт
...
Асинх Функция ДоступнаКомандаСканировать() Экспорт
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Не Ждать ПроинициализироватьКомпоненту() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Асинх Функция ПоместитьФайлСДискаВоВременноеХранилище(ПолноеИмяФайла, АдресФайла = "", УникальныйИдентификатор = Неопределено) Экспорт
	Если Не РасширениеРаботыСФайламиПодключено() Тогда
	...
	РезультатПоместить = Ждать ПоместитьФайлыНаСерверАсинх(,,ЧтоЗагрузить,УникальныйИдентификатор);
	Если РезультатПоместить = Неопределено Или РезультатПоместить.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат РезультатПоместить[0].Хранение;
КонецФункции
...
Асинх Функция ПроинициализироватьКомпоненту() Экспорт
	...
	Если ПараметрыПриложения[ИмяПараметра] = Неопределено Тогда
		КодВозврата = Ждать ПодключитьВнешнююКомпонентуАсинх("ОбщийМакет.КомпонентаTWAIN", "twain", ТипВнешнейКомпоненты.Native);
...
//
Асинх Процедура УстановитьКомпоненту(ОбработчикРезультата) Экспорт
	...
	Если Ждать ПроинициализироватьКомпоненту() Тогда
...
Асинх Процедура УстановитьКомпонентуЗавершение(ПараметрыВыполнения) Экспорт
	
	КомпонентаПодключена = Ждать ПроинициализироватьКомпоненту();
	ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, КомпонентаПодключена);
	
КонецПроцедуры

// Возвращает устройства TWAIN (массив строк).
Асинх Функция ПолучитьУстройства() Экспорт
	...
	// TODO: Возможна проблема из-за Асинх
