...
Процедура ОтправитьУведомлениеОПросроченныхЗадачах(ПисьмоИзНабора)
	...
	// + 27.03.2018 10:19:42 Базунов Д.А. Задача: 
	...
	Если ПисьмоИзНабора.ТипПисьма = "Исполнителю" Тогда
		ТекстТелаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '<b>Не выполненные в срок задачи:<b><br>
			| 
	...
	ИначеЕсли ПисьмоИзНабора.ТипПисьма = "Автору" Тогда
		ТекстТелаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '<b>По введенным задачам прошел крайний срок:<b><br>
			| 
			|%1'"), ПисьмоИзНабора.ТекстПисьма);
		ПараметрыПисьма.Вставить("Тело", ТекстТелаПисьма);
	...
	ИначеЕсли ПисьмоИзНабора.ТипПисьма = "Координатору" Тогда
		ТекстТелаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '<b>Прошел крайний срок по задачам:<b><br>
			| 
			|%1'"), ПисьмоИзНабора.ТекстПисьма);
		ПараметрыПисьма.Вставить("Тело", ТекстТелаПисьма);
...
Функция ВыбратьНовыеЗадачиПоИсполнителям(Знач ДатаВремяОт, Знач ДатаВремяПо)
	...
	//"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	//|	ЗадачаИсполнителя.Ссылка КАК Ссылка,
	//// + 23.01.2018 10:02:05 Базунов Д.А. Задача: 
	...
	|	ЗадачаИсполнителя.ОсновнойОбъектАдресации КАК ОсновнойОбъектАдресации,
	|	ЗадачаИсполнителя.ДополнительныйОбъектАдресации КАК ДополнительныйОбъектАдресации,
	|	ЕСТЬNULL(СоответствиеТочекБизнесПроцессаШаблонам.ШаблонСообщения, ЗНАЧЕНИЕ(Справочник.ШаблоныСообщений.ПустаяСсылка)) КАК ШаблонСообщения
	|ИЗ
	|	Задача.ЗадачаИсполнителя КАК ЗадачаИсполнителя
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИсполнителиЗадач КАК ИсполнителиЗадач
	|		ПО ЗадачаИсполнителя.РольИсполнителя = ИсполнителиЗадач.РольИсполнителя
	|			И ЗадачаИсполнителя.ОсновнойОбъектАдресации = ИсполнителиЗадач.ОсновнойОбъектАдресации
	|			И ЗадачаИсполнителя.ДополнительныйОбъектАдресации = ИсполнителиЗадач.ДополнительныйОбъектАдресации
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеТочекБизнесПроцессаШаблонам КАК СоответствиеТочекБизнесПроцессаШаблонам
	|		ПО ЗадачаИсполнителя.ТочкаМаршрута = СоответствиеТочекБизнесПроцессаШаблонам.Точка
	|ГДЕ
	|	ЗадачаИсполнителя.ПометкаУдаления = ЛОЖЬ
	|	И ЗадачаИсполнителя.Дата > &ДатаВремяОт
	|	И ЗадачаИсполнителя.Дата <= &ДатаВремяПо
...
Функция ОтправитьУведомлениеОНовыхЗадачах(Исполнитель, ЗадачиПоИсполнителю)
	...
	// + 23.01.2018 10:46:06 Базунов Д.А. Задача: 
	
	ШаблонПоУмолчанию = Справочники.ШаблоныСообщений.ШаблонПоУмолчанию;

	Для Каждого Задача Из ЗадачиПоИсполнителю.Строки Цикл
		
		Если ЗначениеЗаполнено(Задача.ШаблонСообщения) Тогда
			Результат = ШаблоныСообщений.СформироватьСообщениеИОтправить(Задача.ШаблонСообщения, Задача.Ссылка, Задача.Ссылка.УникальныйИдентификатор());
		Иначе
			Результат = ШаблоныСообщений.СформироватьСообщениеИОтправить(ШаблонПоУмолчанию, 	 Задача.Ссылка, Задача.Ссылка.УникальныйИдентификатор());
		КонецЕсли;
...
Функция СформироватьПредставлениеЗадачи(ЗадачаСтруктура)
	...
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '<p>%1
		|
		|Крайний срок: %2<br>'") + Символы.ПС,
	...
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Исполнитель) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Исполнитель: %1'"), ЗадачаСтруктура.Исполнитель) + "<br>"+ Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.РольИсполнителя) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Роль: %1'"), ЗадачаСтруктура.РольИсполнителя) + "<br>"+ Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ОсновнойОбъектАдресации) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Основной объект адресации: %1'"), ЗадачаСтруктура.ОсновнойОбъектАдресации) + "<br>"+ Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ДополнительныйОбъектАдресации) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Доп. объект адресации: %1'"), ЗадачаСтруктура.ДополнительныйОбъектАдресации) + "<br>"+ Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Автор) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Автор: %1'"), ЗадачаСтруктура.Автор) + "<br>"+ Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Описание) Тогда
		Результат = Результат + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1'"), ЗадачаСтруктура.Описание) + "<br>"+ Символы.ПС;
	КонецЕсли;
	// + 27.03.2018 10:23:44 Базунов Д.А. Задача: 
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Ссылка) Тогда
		ВнешняяСсылка = "<br><a href="+ОбщегоНазначения.АдресПубликацииИнформационнойБазыВИнтернете() + "#" +  ПолучитьНавигационнуюСсылку(ЗадачаСтруктура.Ссылка)+">Ссылка на задачу</a><br>";
		Результат = Результат + Символы.ПС + ВнешняяСсылка + Символы.ПС;
	КонецЕсли;
	// - 27.03.2018 10:23:44 Базунов Д.А. Задача:
	Возврат Результат + "</p>" + Символы.ПС;
...
Процедура УстановитьПараметрыСпискаМоихЗадач(Список) Экспорт
	...
	Список.Параметры.УстановитьЗначениеПараметра("АвторНеУказан", Пользователи.СсылкаНеуказанногоПользователя());
...
// + 18.04.2018 15:00:26 Базунов Д.А. Задача: 
