...
// Преобразует внутренние форматы вложений к двоичным данным.
...
// Преобразует коды текст письма к формату вставки картинок.
Функция ПривестиТекстПисьмаДляВставкиКартинок(ТекстПисьма)
...
////////////////////////////////////////////////////////////////////////////////
// Состав параметров см. в РаботаСПочтовымиСообщениями.ОтправитьПочтовоеСообщение.
//
		...
		Для Каждого Вложение Из Вложения Цикл
			...
			Иначе // Поддержка обратной совместимости с 2.2.1.
				Если ТипЗнч(Вложение.Значение) = Тип("Структура") Тогда
					...
					КонецЕсли;
				Иначе
					Письмо.Вложения.Добавить(ПривестиВложениеДляВставкиВПисьмо(Вложение.Значение), Вложение.Ключ);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Если ПараметрыПисьма.Свойство("Картинки") И ТипЗнч(ПараметрыПисьма.Картинки) = Тип("Структура") Тогда
		Тело = ПривестиТекстПисьмаДляВставкиКартинок(Тело);
		
		// Запись картинок с именами в качестве идентификаторов.
		Для Каждого КлючИЗначение Из ПараметрыПисьма.Картинки Цикл
			НовоеВложение = Письмо.Вложения.Добавить(ПривестиВложениеДляВставкиВПисьмо(КлючИЗначение.Значение), КлючИЗначение.Ключ);
			НовоеВложение.Идентификатор = КлючИЗначение.Ключ;
		КонецЦикла;
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// Состав параметров см. в РаботаСПочтовымиСообщениями.ЗагрузитьПочтовыеСообщения.
//
Функция ЗагрузитьСообщения(Знач УчетнаяЗапись,
	...
	// + 15.02.2018 15:53:36 Базунов Д.А. Задача: 
	ПараметрыОтбораIMAP = Новый Структура;
	Если ПараметрыЗагрузки.Свойство("ТолькоНеПрочитанные") Тогда
		ПараметрыОтбораIMAP.Вставить("Прочитанные", Ложь);
	КонецЕсли;
	// - 15.02.2018 15:53:36 Базунов Д.А. Задача:
	
	Если ПолучениеЗаголовков Тогда
		НаборПисем = Соединение.ПолучитьЗаголовки(ПараметрыОтбораIMAP);
