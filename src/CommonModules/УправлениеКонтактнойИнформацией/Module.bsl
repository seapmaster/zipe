...
//
Процедура ПриСозданииНаСервере(Форма, Объект, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация", ПоложениеЗаголовкаКИ = "",
	...
	// + 16.01.2018 15:07:34 Базунов Д.А. Задача: 
	ЕстьКолонкаКлючСвязи = (РеквизитыОбъекта.Найти("КлючСвязи") <> Неопределено);
	// - 16.01.2018 15:07:34 Базунов Д.А. Задача:
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект)) Тогда
		...
		// + 16.01.2018 15:08:22 Базунов Д.А. Задача: 
		|	КонтактнаяИнформация.КлючСвязи КАК КлючСвязи,
		// - 16.01.2018 15:08:22 Базунов Д.А. Задача:
	...
	// + 16.01.2018 15:10:37 Базунов Д.А. Задача: 
	Если ЕстьКолонкаКлючСвязи Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, """"" КАК КлючСвязи",
			"КонтактнаяИнформация.КлючСвязи КАК КлючСвязи");
	КонецЕсли;
	// - 16.01.2018 15:10:37 Базунов Д.А. Задача:
...
Процедура ПередЗаписьюНаСервере(Форма, Объект, Отказ = Ложь) Экспорт
	...
	ИмяОбъекта = Объект.Ссылка.Метаданные().Имя;
	ГруппаВидовКИ = ГруппаВидовКонтактнойИнформацииОбъекта(Объект.Ссылка);
	ПредыдущийВид = Неопределено;
	...
	Для Каждого СтрокаТаблицы Из Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Если СтрокаТаблицы.ЭтоРеквизитТабличнойЧасти Тогда
			...
			Для Каждого СтрокаТабличнойЧастиФормы Из ТабличнаяЧастьФормы Цикл
				...
				ЗаписатьКонтактнуюИнформацию(Объект, ЗначенияПолей, ВидИнформации, ТипИнформации, ИдентификаторСтроки,, СтрокаТаблицы.КлючСвязи);
				
			КонецЦикла;
			
		Иначе
			Если ВидИнформации.Родитель <> ГруппаВидовКИ Тогда
				Продолжить;
			КонецЕсли;
			
			ДействуетС = ?(СтрокаТаблицы.Свойство("ДействуетС"), СтрокаТаблицы.ДействуетС, Неопределено);
			ЗаписатьКонтактнуюИнформацию(Объект, СтрокаТаблицы.ЗначенияПолей, ВидИнформации, ТипИнформации,, ДействуетС, СтрокаТаблицы.КлючСвязи);
		КонецЕсли;
...
Функция ОбновитьКонтактнуюИнформацию(Форма, Объект, Результат = Неопределено) Экспорт
	...
	ИначеЕсли Результат.Свойство("ПоменятьМестамиЭлементы") Тогда
		...
		Форма.Элементы[Результат.ПервыйЭлемент].Подсказка = НайденнаяСтрокаПервыйЭлемент.Комментарий;
		Форма.Элементы[Результат.ВторойЭлемент].Подсказка = НайденнаяСтрокаВторойЭлемент.Комментарий;
	КонецЕсли;
...
//  ДанныеЗаполнения	 - Структура - данные для заполнения контактной информации.
...
//
// Параметры:
//  Объект	 - СправочникОбъект, ДокументОбъект  - объект, содержащий контактную информацию.
...
//
// Параметры:
//  Источник			 - ДокументОбъект  - объект, содержащий контактную информацию.
//  ДанныеЗаполнения	 - Структура - данные для заполнения контактной информации.
//  СтандартнаяОбработка - Булево - флаг необходимости выполнения стандартной обработки.
//
...
Функция СтранаАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	Результат = Новый Структура("Ссылка, Код, Наименование, ПолноеНаименованиеСтраны, БуквенныйКодАльфа2, БуквенныйКодАльфа3");
	...
	ДанныеСтраны = Справочники.ОКСМ.ДанныеОКСМ(, Результат.Наименование);
...
Функция АдресВФорматеКЛАДР(Знач Данные) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Данные) Тогда
		...
		Результат = УправлениеКонтактнойИнформациейКлиентСервер.СтруктураЗначенийПолей(
			ПредыдущийФорматКонтактнойИнформацииXML(Данные));
			Представление = ПредставлениеКонтактнойИнформации(Данные);
			
	ИначеЕсли ПустаяСтрока(Данные) Тогда
		// Генерируем пустую структуру по виду
		Результат =  УправлениеКонтактнойИнформациейКлиентСервер.СтруктураКонтактнойИнформацииПоТипу(
			Перечисления.ТипыКонтактнойИнформации.Адрес);
		Представление = "";
	КонецЕсли;
	
	
	Если Результат.Свойство("Страна") 
		И СтрСравнить(Результат.Страна, Справочники.ОКСМ.Россия.Наименование) = 0 Тогда
...
//        * Ссылка             - СправочникСсылка.ОКСМ, Неопределено - соответствующий элемент страны мира.
//        * Наименование       - Строка - часть описания страны.
//        * Код                - Строка - часть описания страны.
//        * НаименованиеПолное - Строка - часть описания страны.
//        * КодАльфа2          - Строка - часть описания страны.
...
Функция ДанныеСтраныМира(Знач КодСтраны = Неопределено, Знач Наименование = Неопределено) Экспорт
	Возврат Справочники.ОКСМ.ДанныеОКСМ(КодСтраны, Наименование);
КонецФункции
...
Функция ДанныеКлассификатораСтранМираПоКоду(Знач Код, Знач ТипКода = "КодСтраны") Экспорт
	Возврат Справочники.ОКСМ.ДанныеКлассификатораСтранМираПоКоду(Код, ТипКода);
КонецФункции
...
Функция ДанныеКлассификатораСтранМираПоНаименованию(Знач Наименование) Экспорт
	Возврат Справочники.ОКСМ.ДанныеКлассификатораСтранМираПоНаименованию(Наименование);
КонецФункции
...
//                                                                      информации.
//        * ХранитьИсториюИзменений - Булево -                          Признак хранения истории изменений вида контактной информации.
...
Процедура ЗаписатьКонтактнуюИнформацию(Объект, ЗначенияПолей, ВидИнформации, ТипИнформации, ИдентификаторСтроки = 0, Дата = Неопределено, КлючСвязи = "") Экспорт
	...
	// + 16.01.2018 16:22:59 Базунов Д.А. Задача:
	Если НЕ КлючСвязи = "" И ОбщегоНазначения.ЕстьРеквизитОбъекта("КлючСвязи", Объект.Метаданные().ТабличныеЧасти.КонтактнаяИнформация) Тогда
...
Процедура ИзменитьКомментарий(Форма, ИмяРеквизита, ИмяЭлементаДляРазмещения)
	...
	ПолеВвода.Подсказка = НайденнаяСтрока.Комментарий;
...
Процедура ДобавитьСтрокуКонтактнойИнформации(Форма, Результат, ИмяЭлементаДляРазмещения, ЭтоНовыйВидКИ = Ложь, КоличествоАдресов = Неопределено, СледующаяСтрока = Неопределено)
	...
	ПолеВвода = ПолеВвода(Форма, ГруппаСтроки, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения, ЭтоНовыйВидКИ, ОбязательноеЗаполнение);
	...
	Если ЕстьКомментарий Тогда
		ПолеВвода.Подсказка = Результат.Комментарий;
	КонецЕсли;
...
Функция ПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	//++ 20170307 07.03.17 Базунов Дмитрий 
	//Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
	//	
...
Процедура ПроверитьНаличиеРеквизитовКонтактнойИнформации(Форма, МассивДобавляемыхРеквизитов)
	...
	Если СоздатьТаблицуКонтактнойИнформации Тогда
		...
		// + 16.01.2018 16:17:57 Базунов Д.А. Задача: 
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("КлючСвязи", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(36)), ИмяОписания));
		// - 16.01.2018 16:17:57 Базунов Д.А. Задача:
...
Процедура ДобавитьРеквизитВОписание(Форма, СтрокаКИ, ЭтоНовыйВидКИ, ЭтоРеквизитТабличнойЧасти = Ложь, ЗаполнятьЗначениеРеквизита = Истина, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация")
	...
	// + 16.01.2018 16:07:26 Базунов Д.А. Задача: 
	НоваяСтрока.КлючСвязи	  = СтрокаКИ.КлючСвязи;
	// - 16.01.2018 16:07:26 Базунов Д.А. Задача:
	...
	Если ЭтоНовыйВидКИ И СтруктураВидаКИ.РазрешитьВводНесколькихЗначений И Не ЭтоРеквизитТабличнойЧасти Тогда
		ПараметрыКонтактнойИнформации = ПараметрыКонтактнойИнформацииФормы(Форма.ПараметрыКонтактнойИнформации, ИмяЭлементаДляРазмещения);
		ПараметрыКонтактнойИнформации.СписокДобавляемыхЭлементов.Добавить(СтруктураВидаКИ, СтрокаКИ.Вид.Наименование);
	КонецЕсли;
...
Функция ПараметрыКонтактнойИнформации(Форма, ИмяЭлементаДляРазмещения, ПоложениеЗаголовкаКИ, ОтложеннаяИнициализация, ИсключаемыеВиды)
...
Функция ГруппаВидовКонтактнойИнформацииОбъекта(СсылкаНаОбъект)
...
Процедура СоздатьДействие(Форма, ВидКонтактнойИнформации, ИмяРеквизита, ГруппаДействий, КоличествоАдресов, ЕстьКомментарий = Ложь, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация")
	...
	Если СоздаватьДействиеДляТипа[Тип] = Истина Тогда
		...
		//++ 2017030701 07.03.17 Базунов Дмитрий 
		Если НЕ Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			Возврат;
		КонецЕсли;
		//-- 2017030701 07.03.17 Базунов Дмитрий  
