...
Функция ПараметрыАдминистрирования() Экспорт
	...
	Иначе
		Если ПараметрыАдминистрированияИБ.Свойство("РазрешеноОткрытиеВнешнихОтчетовИОбработок") Тогда
			ПараметрыАдминистрированияПоУмолчанию.Вставить("РазрешеноОткрытиеВнешнихОтчетовИОбработок");
		КонецЕсли;
		Если ПараметрыАдминистрированияИБ.Свойство("ПринятоРешениеПоОткрытиюВнешнихОтчетовИОбработок") Тогда
			ПараметрыАдминистрированияПоУмолчанию.Вставить("ПринятоРешениеПоОткрытиюВнешнихОтчетовИОбработок");
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики) Экспорт
	...
	ПодключаемыеКоманды.ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.СтруктураПодчиненности") Тогда
		...
		МодульСтруктураПодчиненностиСлужебный.ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ТекущиеДела") Тогда
		МодульТекущиеДелаСлужебный = ОбщегоНазначения.ОбщийМодуль("ТекущиеДелаСлужебный");
		МодульТекущиеДелаСлужебный.ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики);
	КонецЕсли;
...
// Обновляет вспомогательные данные для работы программы, хранимые в неразделенных константах.
//
// Параметры:
...
//                      если отличаются от ранее установленных.
//  ЕстьИзменения     - Булево - (возвращаемое значение). Устанавливается Истина, если старое и новое
//                      значения параметра не совпадают.
//  ТолькоПроверка    - Булево - если указать Истина, тогда обновление не будет выполнено, а будет
//                      выполнено только сравнение старого и нового значения параметра, а также
//                      будет установлен параметр ЕстьИзменения, если значения не совпадают.
//  Параметры         - ФиксированнаяСтруктура - можно получить с помощью функции ПараметрыРаботыПрограммы.
//                    - Неопределено - (возвращаемое значение) функции ПараметрыРаботыПрограммы.
//
Процедура ОбновитьПараметрРаботыПрограммы(ИмяКонстанты, ИмяПараметра, ЗначениеПараметра,
			ЕстьИзменения = Ложь, ТолькоПроверка = Ложь, Параметры = Неопределено) Экспорт
	
	Если Параметры = Неопределено Тогда
		Параметры = ПараметрыРаботыПрограммы(ИмяКонстанты);
	КонецЕсли;
	
	СохраненноеЗначение = Неопределено;
	
	Если Параметры.Свойство(ИмяПараметра) Тогда
		СохраненноеЗначение = Параметры[ИмяПараметра];
		
		Если Не ОбщегоНазначения.ДанныеСовпадают(ЗначениеПараметра, СохраненноеЗначение) Тогда
			СохраненноеЗначение = Неопределено;
		КонецЕсли;
	КонецЕсли;
	...
	УстановитьПараметрРаботыПрограммы(ИмяКонстанты, ИмяПараметра, ЗначениеПараметра);
...
Процедура УстановитьПараметрРаботыРасширения(ИмяПараметра, Хранилище) Экспорт
	...
	РегистрыСведений.ПараметрыРаботыВерсийРасширений.УстановитьПараметрРаботыРасширения(ИмяПараметра, Хранилище);
...
//
Функция ОбъектМетаданныхНеразделенногоРасширения(ОбъектМетаданных)
	
	Если Справочники.ВерсииРасширений.УстановленныеРасширенияДоступны() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ИдентификаторОбъектаМетаданных(ОписаниеОбъектаМетаданных) Экспорт
	...
	Если ТипЗнч(ОбъектМетаданных) = Тип("ОбъектМетаданных")
		И ОбъектМетаданныхНеразделенногоРасширения(ОбъектМетаданных) Тогда
...
Процедура ПриДобавленииОбработчиковСлужебныхСобытийБазовойФункциональности(КлиентскиеОбработчики, СерверныеОбработчики)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ВыгрузкаЗагрузкаДанных") Тогда
		...
		].Добавить("СтандартныеПодсистемыСервер");
	КонецЕсли;
...
Процедура ПроверитьОбновитьПараметрыРаботыПрограммы(ЕстьКритичныеИзменения, РежимОбновления, БезИзменений, ВФоне)
	...
	Если НЕ БезИзменений.Свойство("ПользователиПараметрыРаботы") Тогда
		...
		КонецЕсли;
		Если ВФоне Тогда
			ДлительныеОперации.СообщитьПрогресс(80);
		КонецЕсли;
	КонецЕсли;
