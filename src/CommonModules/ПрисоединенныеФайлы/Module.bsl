...
Функция ПолучитьДанныеФайла(Знач ПрисоединенныйФайл,
	...
	Если ПолучатьСсылкуНаДвоичныеДанные Тогда
		...
		// Хранилище файлов
		Если Константы.ВыгружатьФайлыВоВнешнееХранилище.Получить() И ДвоичныеДанные.Размер() = 0 Тогда
			ОбменСХранилищемФайлов.ПоместитьДвоичныеДанныеИзХранилищаФайловВКеш(ПрисоединенныйФайл);
			ДвоичныеДанные = ПолучитьДвоичныеДанныеФайла(ПрисоединенныйФайл);		
		КонецЕсли; // Если Константы.ВыгружатьФайлыВоВнешнееХранилище.Получить() И ДвоичныеДанные.Размер() = 0 Тогда
...
Функция ДобавитьПрисоединенныйФайл(ПараметрыФайла,
	...
	//+ 23.03.2017 13:16:28 Савинов Александр Задача: ОПЗ 2
	...
	Если ДопПараметры.Свойство("ПрисоединенныйФайлСсылка") Тогда
		
		НоваяСсылкаНаФайл = ДопПараметры.ПрисоединенныйФайлСсылка;
	...
	//- 23.03.2017 13:16:31 Савинов Александр Задача: ОПЗ 2	
	...
	//+ 22.03.2017 13:45:55 Савинов Александр Задача: ОПЗ 2
	
	Если ДопПараметры.Свойство("Чертеж") Тогда
		
		ПрисоединенныйФайл.Чертеж = ДопПараметры.Свойство("Чертеж");
	...
	// Хранищище Файлов
	РегистрыСведений.РазборХранилищеФайлов.ДобавитьЗапись(ПрисоединенныйФайл.Ссылка);
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	//+ 04.05.2017 14:27:53 Савинов Александр Задача: ОПЗ 4
	...
	ИменаСправочников = ПрисоединенныеФайлыСлужебный.ИменаСправочниковХраненияФайлов(ТипЗнч(Источник.Ссылка), Истина);
	...
	//- 04.05.2017 14:27:55 Савинов Александр Задача: ОПЗ 4
