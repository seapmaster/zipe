...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач Параметры = Неопределено) Экспорт
	...
	//+ 14.04.2017 8:46:34 Савинов Александр Задача: ОПЗ 2
	
	Если ПараметрыВыполнения.УспешныеЗамены.Количество() > 0 Тогда
	
		ОбщиеФункцииСервер.ОбработатьУдалениеДублей(ПараметрыВыполнения.УспешныеЗамены);
		
	КонецЕсли;
		
	//- 14.04.2017 8:46:38 Савинов Александр Задача: ОПЗ 2
...
Функция ПредметСтрокой(СсылкаНаПредмет) Экспорт
	...
	Если ТипЗнч(СсылкаНаПредмет) = Тип("ДокументСсылка.ЗаявкаФСВТС") Тогда
		Результат = "Заявка №" + СсылкаНаПредмет.НомерЗаказчика + " от " + СсылкаНаПредмет.Заказчик;
	КонецЕсли; // Если ТипЗнч(СсылкаНаПредмет) = Тип("ДокументСсылка.ЗаявкаФСВТС") Тогда
...
//
// Параметры:
//  Параметры                       - Структура - настройки подключения (необходимо использовать функцию ПараметрыПодключенияWSПрокси):
...
//   * ИспользоватьАутентификациюОС - Булево - (необязательный) включает использование авторизации NTLM или Negotiate на сервере.
...
Функция СоздатьWSПрокси(ПереданныеПараметры) Экспорт
	...
	Если ДелатьКонтрольныйВызов И Таймаут <> Неопределено И Таймаут > 20 Тогда
		ПараметрыПодключения.Таймаут = 3;
		ПараметрыПодключенияСтрокой = ЗначениеВСтрокуXML(ПараметрыПодключения);
		WSПроксиPing = ОбщегоНазначенияПовтИсп.СоздатьWSПрокси(ПараметрыПодключенияСтрокой);
	...
	ПараметрыПодключенияСтрокой = ЗначениеВСтрокуXML(ПараметрыПодключения);
	Возврат ОбщегоНазначенияПовтИсп.СоздатьWSПрокси(ПараметрыПодключенияСтрокой);
...
Функция ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Экспорт
	Если ОбъектМетаданных = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	Возврат ОбщегоНазначенияПовтИсп.ДоступностьОбъектовПоОпциям()[ОбъектМетаданных.ПолноеИмя()] <> Ложь;
КонецФункции
...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	ОбщиеПараметры.Вставить("МинимальноНеобходимаяВерсияПлатформы", "8.3.8.1652");
...
// Возвращает адрес информационной базы для для пользователей, работающих в базе через Интернет.
//
// Возвращаемое значение:
...
// Возвращает адрес информационной базы для для пользователей, работающих в базе из локальной сети.
//
// Возвращаемое значение:
//   Строка - значение константы.
//
...
// Возвращает адрес входа в программу для указанного пользователя.
//
// Параметры:
...
Функция ПолучитьВерсииИнтерфейсаВКэш(Знач ПараметрыПодключения, Знач ИмяИнтерфейса)
	...
	ПараметрыПодключения.Вставить("Таймаут", 3);
	
	ПроксиВерсионирования = СоздатьWSПрокси(ПараметрыПодключения);
