...
Функция ПолучитьПотокШаблонаWORD(ИмяШаблона)
	Результат = Новый ПотокВПамяти;
	Макет = Обработки.ХранилищеМакетов.ПолучитьМакет(ИмяШаблона);
	Макет.Записать(Результат);
	Возврат	Результат;
КонецФункции // ПолучитьПотокШаблонаWORD

Функция ПолучитьВременнуюПапкуWORD()
...
КонецФункции // ПолучитьВременнуюПапкуWORD

Процедура ВыполнитьЗаменуПараметровWORD(ВременнаяПапка, СписокПараметров, ИмяДокумента)

	Текст = Новый ЧтениеТекста(ВременнаяПапка + "\word\" + ИмяДокумента +".xml", КодировкаТекста.UTF8);
	РезТекст = Новый ЗаписьТекста(ВременнаяПапка + "\word\" + ИмяДокумента + "1.xml", КодировкаТекста.UTF8);
	...
	Попытка
		УдалитьФайлы(ВременнаяПапка + "\word\"+ИмяДокумента+".xml");
		ПереместитьФайл(ВременнаяПапка + "\word\"+ИмяДокумента+"1.xml",ВременнаяПапка + "\word\"+ИмяДокумента+".xml");
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось подготовить документ: " + ОписаниеОшибки());
	КонецПопытки;
	
КонецПроцедуры // ВыполнитьЗаменуПараметровWORD

Функция РаспаковатьWORDВоВременнуюПапку(ПотокШаблон, ВременнаяПапка)
	Результат = Истина;
	ЧтениеZIP = Новый ЧтениеZipФайла(ПотокШаблон);
	ЧтениеZIP.ИзвлечьВсе(ВременнаяПапка, РежимВосстановленияПутейФайловZIP.Восстанавливать);
	ЧтениеZIP.Закрыть();
	Возврат	Результат;
КонецФункции // РаспаковатьWORDВоВременнуюПапку

Функция УпаковатьWORD(ВременнаяПапка)
	Результат = Новый ПотокВПамяти;
	ЗаписьZIP = Новый ЗаписьZipФайла(Результат);
	...
	ЗаписьZIP.Записать();
	Возврат Результат;
КонецФункции // УпаковатьWORD

Функция ОбработатьШаблонWORD(ИмяШаблона, СписокПараметров, ДополнительныеДокументы = Неопределено);

	ПотокШаблон = ПолучитьПотокШаблонаWORD(ИмяШаблона);
	
	ВременнаяПапка = ПолучитьВременнуюПапкуWORD();
	
	РаспаковатьWORDВоВременнуюПапку(ПотокШаблон, ВременнаяПапка);
	
	ИмяДокумента = "document";
	ВыполнитьЗаменуПараметровWORD(ВременнаяПапка, СписокПараметров, ИмяДокумента);
	
	Если Не ДополнительныеДокументы = Неопределено Тогда
		Для Каждого ИмяДокумента Из ДополнительныеДокументы Цикл
			ВыполнитьЗаменуПараметровWORD(ВременнаяПапка, СписокПараметров, ИмяДокумента);
		КонецЦикла; // Для Каждого ИмяДокумента Из ДополнительныеДокументы Цикл
	КонецЕсли; // Если Не ДополнительныеДокументы = Неопределено Тогда
	
	ПотокРезультат = УпаковатьWORD(ВременнаяПапка);
	
	АдресХранилища = ПоместитьВоВременноеХранилище(ПотокРезультат.ЗакрытьИПолучитьДвоичныеДанные(), Новый УникальныйИдентификатор);
...
КонецФункции // ОбработатьШаблонWORD
...
Функция ПодговитьПисьмоВыгрузкаПСДляПоставщиков(СписокПараметров) Экспорт
	Возврат ОбработатьШаблонWORD("ВыгрузкаПСПоставщикам_Письмо", СписокПараметров);
КонецФункции // ПодговитьПисьмоВыгрузкаПСДляПоставщиков
...
Функция ПодготовитьПисьмоТКПКонтракт(ИмяМакета, СписокПараметров, ДополнительныеДокументы = Неопределено) Экспорт
	Возврат ОбработатьШаблонWORD(ИмяМакета, СписокПараметров, ДополнительныеДокументы);	
КонецФункции // ПодготовитьПисьмоТКПКонтракт

Функция ПодготовитьПисьмоФСВТСКонтракт(СписокПараметров) Экспорт
	Возврат ОбработатьШаблонWORD("Контракт_ПисьмоФСВТС", СписокПараметров);
КонецФункции // ПодготовитьПисьмоФСВТСКонтракт
...
Функция ПодготовитьПисьмоФСТВСЗаявка(СписокПараметров) Экспорт
	Возврат ОбработатьШаблонWORD("Заявка_ПисьмоФСВТС", СписокПараметров);
КонецФункции // ПодготовитьПисьмоФСТВСЗаявка
