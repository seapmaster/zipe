...
Функция КонструкторСообщения(ПараметрыШаблона = Неопределено) Экспорт
	...
	Сообщение.Вставить("ДополнительныеПараметры", ПараметрыШаблона);
	Сообщение.Вставить("СообщенияПользователю");
...
Функция СведенияОШаблоне(ПараметрыШаблона) Экспорт
	...
	НастройкиШаблоновСообщений = ПриОпределенииНастроек();
...
Функция ПриОпределенииНастроек() Экспорт
	...
	ОбщиеРеквизитыДерево = ОпределитьОбщиеРеквизиты();
	Настройки.ОбщиеРеквизиты = ОбщиеРеквизиты(ОбщиеРеквизитыДерево);
...
Функция ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(Владелец, Сведения, ИмяФайла, Размер, КоличествоПустыхИменВоВложениях = 0) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		Возврат МодульПрисоединенныеФайлы.ДобавитьПрисоединенныйФайл(ПараметрыФайла, АдресВоВременномХранилище, "", Описание);
	КонецЕсли;
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						...
						УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru = 'Ошибка при создании внешний печатной формы.'"));
					Прервать;
				КонецПопытки;
...
Функция ПараметрыШаблона(Шаблон) Экспорт
	...
	Результат = ОписаниеПараметровШаблона();
	...
	Если ТипЗнч(Шаблон) = Тип("ДанныеФормыСтруктура") Тогда
		...
		КонецЕсли;
		Результат.ШаблонПоВнешнейОбработке = Шаблон.ШаблонПоВнешнейОбработке;
		Результат.ВнешняяОбработка         = Шаблон.ВнешняяОбработка;
		...
		Если СтрНачинаетсяС(Результат.Назначение, "Служебный") Тогда
	...
	ИначеЕсли ТипЗнч(Шаблон) = Тип("СправочникСсылка.ШаблоныСообщений") Тогда
		...
		Если СведенияОШаблоне.Количество() > 0 Тогда
			...
			Результат.ВнешняяОбработка         = СтрокаСведенийОШаблоне.ВнешняяОбработка;
			Результат.ШаблонПоВнешнейОбработке = СтрокаСведенийОШаблоне.ШаблонПоВнешнейОбработке;
...
Функция ОписаниеПараметровШаблона() Экспорт
	...
	Результат.Вставить("ФорматПисьма", Перечисления.СпособыРедактированияЭлектронныхПисем.HTML);
...
Функция СписокПараметров(ПараметрыТекстаСообщения, Префикс = "")
	...
	Для каждого Реквизит Из ПараметрыТекстаСообщения Цикл
		Если ТипЗнч(Реквизит.Значение) = Тип("Соответствие") Тогда
			СписокРеквизитов = СписокРеквизитов + СписокПараметров(Реквизит.Значение, Реквизит.Ключ + "."); // + " КАК " + СтрЗаменить(Реквизит.Ключ, ".", "_");
		Иначе
...
Функция ЗаполнитьПараметрыСообщения(ПараметрыШаблона, ПараметрыТекстаСообщения, ПараметрыОтправки)
	...
	Если ПараметрыОтправки.ДополнительныеПараметры.Свойство("ПроизвольныеПараметры") Тогда
		Для каждого ПроизвольныйПараметр Из ПараметрыОтправки.ДополнительныеПараметры.ПроизвольныеПараметры Цикл
			
			Если НЕ ЗначениеЗаполнено(ПроизвольныйПараметр.Значение) Тогда
				Продолжить;
			КонецЕсли;
			
			Если ТипЗнч(ПроизвольныйПараметр.Значение) = Тип("Строка")
				Или ТипЗнч(ПроизвольныйПараметр.Значение) = Тип("Дата") Тогда
				...
				Если ТипЗнч(ПроизвольныеРеквизиты ) = Тип("Соответствие") Тогда
					ПараметрыТекстаСообщения[ШаблоныСообщенийКлиентСервер.ЗаголовокПроизвольныхПараметров()][ПроизвольныйПараметр.Ключ] = ПроизвольныйПараметр.Значение;
				КонецЕсли;
			Иначе
				ИмяОбъекта = ПроизвольныйПараметр.Значение.Метаданные().Имя;
				Если ПараметрыТекстаСообщения[ИмяОбъекта] <> Неопределено Тогда
					ЗаполнитьРеквизитыПоПредмету(ПараметрыТекстаСообщения[ИмяОбъекта], ПроизвольныйПараметр.Значение);
					ЗаполнитьРеквизитыСвойствИКонтактнойИнформации(ПараметрыТекстаСообщения[ИмяОбъекта], ПроизвольныйПараметр.Значение);
					Сообщение.ЗначенияРеквизитов = ПараметрыТекстаСообщения[ИмяОбъекта];
				Иначе
...
Процедура ОбработатьHTMLДляФорматированногоДокумента(ПараметрыШаблона, СформированноеСообщение, ПреобразовыватьHTMLДляФорматированногоДокумента, СписокФайлов = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		Если СписокФайлов = Неопределено Тогда
			...
			МодульПрисоединенныеФайлы.ПолучитьПрикрепленныеФайлыКОбъекту(ПараметрыШаблона.Шаблон, СписокФайлов);
		КонецЕсли;
		...
		Для каждого Картинка Из ДокументHTML.Картинки Цикл
			...
			Для Каждого ПрисоединенныйФайл Из СписокФайлов Цикл
				Если СтрЧислоВхождений(АтрибутИсточникКартинки.Значение, ПрисоединенныйФайл.ИДФайлаЭлектронногоПисьма) > 0 Тогда
					ДанныеФайла = МодульПрисоединенныеФайлы .ПолучитьДанныеФайла(ПрисоединенныйФайл.Ссылка, ПараметрыШаблона.УникальныйИдентификатор);
...
Процедура ДобавитьРеквизитыСвойств(МетаданныеОбъект, Префикс, Реквизиты, СведенияОИсключаемыхРеквизитах, СведенияОСпискеРеквизитов)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		...
		Если ПолучатьДопРеквизиты Или ПолучатьДопСведения Тогда
			Свойства = МодульУправлениеСвойствами.ПолучитьСписокСвойств(ПустаяСсылка, ПолучатьДопРеквизиты, ПолучатьДопСведения);
...
Процедура ДобавитьРеквизитПоМетаданнымОбъекта(Реквизиты, Реквизит, Префикс, СведенияОСпискеРеквизитов, СведенияОИсключаемыхРеквизитах);
	...
	// + 22.01.2018 15:08:17 Базунов Д.А. Задача: 
...
Функция ЗначенияРеквизитовСвойств(Предмет)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		...
		Если ПолучатьДопРеквизиты Или ПолучатьДопСведения Тогда
			ЗначенияСвойств = МодульУправлениеСвойствами.ПолучитьЗначенияСвойств(Предмет, ПолучатьДопРеквизиты, ПолучатьДопСведения);
		КонецЕсли;
...
Функция ЗначенияРеквизитовКонтактнойИнформации(Предмет)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		Если ВидыКонтактнойИнформации.Количество() > 0 Тогда
			КонтактнаяИнформацияОбъектов = МодульУправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(ОбъектыСКонтактнойИнформацией);
		КонецЕсли;
	КонецЕсли;
...
Функция ОбщиеРеквизиты(Реквизиты)
...
Процедура ДобавитьПрисоединенныеФайлыВоВложения(Знач ПараметрыОтправки, Знач Сообщение)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		МодульПрисоединенныеФайлы.ПолучитьПрикрепленныеФайлыКОбъекту(ПараметрыОтправки.Шаблон, СписокПрисоединенныхФайлов);
		...
		Для каждого ПрисоединенныйФайл Из СписокПрисоединенныхФайлов Цикл
			Если ПустаяСтрока(ПрисоединенныйФайл.ИДФайлаЭлектронногоПисьма) Тогда
				ОписаниеФайла = МодульПрисоединенныеФайлы.ПолучитьДанныеФайла(ПрисоединенныйФайл.Ссылка, ПараметрыОтправки.УникальныйИдентификатор);
...
Процедура ЗаполнитьПолучателейСообщения(ПараметрыОтправки, ПараметрыШаблона, Результат, МенеджерОбъекта)
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо" Тогда
		Получатели = СформироватьПолучателейПоУмолчанию(ПараметрыОтправки.Предмет, Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
		...
		КонецЦикла;
	Иначе
		Получатели = СформироватьПолучателейПоУмолчанию(ПараметрыОтправки.Предмет, Перечисления.ТипыКонтактнойИнформации.Телефон);
...
Функция СформироватьПолучателейПоУмолчанию(Предмет, ТипКонтактнойИнформации);
	...
	// Наше заполнение, если у объекта есть контактная информация.
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		Если ВидыКонтактнойИнформацияОбъекта.Количество() > 0 Тогда
			КонтактнаяИнформацияОбъектов = МодульУправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(ОбъектыСКонтактнойИнформацией, ТипКонтактнойИнформации);
			Если КонтактнаяИнформацияОбъектов.Количество() > 0 Тогда
				Для каждого КонтактнаяИнформацияОбъекта Из КонтактнаяИнформацияОбъектов Цикл
					...
					НоваяСтрока.Представление = Строка(КонтактнаяИнформацияОбъекта.Объект) + " <" + КонтактнаяИнформацияОбъекта.Представление + ">";
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
