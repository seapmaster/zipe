...
//      Регион                  - Строка - Значение поле региона, только если указан тип контактной информации Адрес. Актуально для стран ЕАЭС.
...
//    ИсточникКонтактнойИнформации - ЛюбаяСсылка - Ссылка на источник являющийся владельцем адрес электронной почты.
//
Процедура СоздатьЭлектронноеПисьмо(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено, ИсточникКонтактнойИнформации = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		...
		Получатель.Добавить(Новый Структура("Адрес, Представление, ИсточникКонтактнойИнформации", АдресПочты, Строка(ИсточникКонтактнойИнформации), ИсточникКонтактнойИнформации));
...
// Параметры:
//  Форма						   - УправляемаяФорма - Форма владельца контактной информации.
//  ПараметрыКонтактнойИнформации  - Структура -параметры контактной информации.
//
...
// 
// Возвращаемое значение:
//  Неопределено -  не используется, оставлено для обратной совместимости.
//
...
Процедура ПредставлениеНачалоВыбораЗавершение(Знач РезультатЗакрытия, Знач ДополнительныеПараметры) Экспорт
	...
	Если НоваяСтрока.Свойство("ХранитьИсториюИзменений") И НоваяСтрока.ХранитьИсториюИзменений Тогда
		...
		ИначеЕсли НайденныеСтроки.Количество() = 1 Тогда
			...
			НоваяСтрока.ДействуетС = Дата(1, 1, 1);
		КонецЕсли;
...
Процедура НачалоВыбораУлицы(Владелец, ИдентификаторНаселенногоПункта, ТекущееЗначение, ДополнительныеПараметры) Экспорт
	...
	Параметры.Вставить("НаселенныйПунктДетально", ДополнительныеПараметры.НаселенныйПунктДетально);
...
Процедура ПредложениеЗагрузкиКлассификатора(Знач Текст = "", Знач Регион = Неопределено) Экспорт
	...
	Если НеЗагружатьАдресныйКлассификатор = Неопределено ИЛИ НеЗагружатьАдресныйКлассификатор = Ложь Тогда
		...
		ОткрытьФорму("Обработка.ВводКонтактнойИнформации.Форма.ЗагрузкаАдресногоКлассификатора", ПараметрыФормы, ЭтотОбъект,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура СоздатьПисьмоПоКонтактнойИнформацииЗавершение(Действие, АдресПочты) Экспорт
	...
	ПерейтиПоНавигационнойСсылке("mailto:" + АдресПочты);
