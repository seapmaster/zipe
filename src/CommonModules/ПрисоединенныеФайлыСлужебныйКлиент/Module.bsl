...
Асинх Процедура ДобавитьФайлыРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		Если Не ДополнительныеПараметры.Свойство("ПолноеИмяФайла") Тогда
			...
			РезультатВыборать = Ждать ВыборФайла.ВыбратьАсинх(); 
			
			Если Не РезультатВыборать  = Неопределено Тогда
				ВыбранныеФайлы = РезультатВыборать;
			КонецЕсли;
		Иначе
		...
		Если ВыбранныеФайлы.Количество() > 0  Тогда
			...
			ПрисоединенныеФайлыМассив = Ждать ПоместитьВыбранныеФайлыВХранилище(
				ВыбранныеФайлы,
				ВладелецФайла,
				ПрисоединенныеФайлыМассив,
				ИдентификаторФормы,
				ИмяСоздаваемогоФайла,
				?(ДополнительныеПараметры.Свойство("ДопПараметры"), ДополнительныеПараметры.ДопПараметры, Неопределено)); // 22.03.2017 13:45:55 Савинов Александр
...
Асинх Функция ИзвлечьТекстВоВременноеХранилище(ПолноеИмяФайла, УникальныйИдентификатор = "", Отказ = Ложь,
	...
	#Если Не ВебКлиент Тогда
		...
		Текст = ФайловыеФункцииСлужебныйКлиентСервер.ИзвлечьТекст(ПолноеИмяФайла, Отказ, Кодировка);
		...
		РезультатЗагрузки = Ждать ФайловыеФункцииСлужебныйКлиент.ПоместитьФайлСДискаВоВременноеХранилище(ИмяВременногоФайла, , УникальныйИдентификатор);
...
Асинх Функция ПолучитьФайлВРабочийКаталог(Знач АдресДвоичныхДанныхФайла,
	...
	Попытка
		Ждать СоздатьКаталогАсинх(КаталогСохранения);
	Исключение
	...
	Если Ждать Файл.СуществуетАсинх() Тогда
		...
		Ждать УдалитьФайлыАсинх(КаталогСохранения + ИмяФайла);
	КонецЕсли;
	...
	РезультатПолучить = Ждать ПолучитьФайлыССервераАсинх(ПолучаемыеФайлы, КаталогСохранения);
	Если РезультатПолучить.Количество() > 0 Тогда
		ПолноеИмяФайлаНаКлиенте = РезультатПолучить[0].Имя;
		Файл = Новый Файл(ПолноеИмяФайлаНаКлиенте);
		Ждать Файл.УстановитьУниверсальноеВремяИзмененияАсинх(ДатаМодификацииУниверсальная);
		
		Результат = Новый Структура;
		Результат.Вставить("ФайлМожноОткрывать", Истина);
		Результат.Вставить("ПолноеИмяФайлаНаКлиенте", ПолноеИмяФайлаНаКлиенте);
...
// См. одноименную функцию в общем модуле ПрисоединенныеФайлыКлиент.
Асинх Функция ПоместитьФайлВХранилище(Знач ПутьКФайлу, Знач ИдентификаторФормы)
	...
	Если Не ФайловыеФункцииСлужебныйКлиентСервер.ОбщиеНастройкиРаботыСФайлами().ИзвлекатьТекстыФайловНаСервере Тогда
		АдресВременногоХранилищаТекста = Ждать ИзвлечьТекстВоВременноеХранилище(ПутьКФайлу, ИдентификаторФормы);
	КонецЕсли;
	
	ПомещаемыеФайлы = Новый Массив;
	ПомещаемыеФайлы.Добавить(Новый ОписаниеПередаваемогоФайла(ПутьКФайлу));
	
	РезультатПоместить = Ждать ПоместитьФайлыНаСерверАсинх(,,ПомещаемыеФайлы, ИдентификаторФормы);
	Если РезультатПоместить = Неопределено Тогда
		...
		Возврат Результат;
	Иначе
		ПомещенныеФайлы= РезультатПоместить;	
	КонецЕсли;
	
	Результат.Вставить("ФайлПомещенВХранилище", Истина);
	Результат.Вставить("ДатаМодификацииУниверсальная",   Ждать Файл.ПолучитьУниверсальноеВремяИзмененияАсинх());
	Результат.Вставить("АдресФайлаВоВременномХранилище", ПомещенныеФайлы[0].Адрес);
...
//
Асинх Процедура ДобавитьФайлыПеретаскиванием(Знач ВладелецФайла, Знач ИдентификаторФормы, Знач МассивИменФайлов) Экспорт
	ПрисоединенныеФайлыМассив = Новый Массив;
	ПрисоединенныеФайлыМассив = Ждать ПоместитьВыбранныеФайлыВХранилище(
		МассивИменФайлов,
		ВладелецФайла,
...
Асинх Процедура ПоместитьРедактируемыйФайлНаДискеВХранилищеРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	
	ДанныеФайла = ДополнительныеПараметры.ДанныеФайла;
	ИдентификаторФормы = ДополнительныеПараметры.ИдентификаторФормы;
	
	Если РасширениеРаботыСФайламиПодключено Тогда
		РабочийКаталогПользователя = Ждать ФайловыеФункцииСлужебныйКлиент.РабочийКаталогПользователя();
		...
		Файл = Новый Файл(ПолноеИмяФайлаНаКлиенте);
		Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ПоместитьРедактируемыйФайлНаДискеВХранилищеРасширениеПредложеноЗавершение", ЭтотОбъект, Новый Структура("ДополнительныеПараметры, ИдентификаторФормы, ПолноеИмяФайлаНаКлиенте", ДополнительныеПараметры, ИдентификаторФормы, ПолноеИмяФайлаНаКлиенте)));
	Иначе
...
КонецПроцедуры // ПоместитьРедактируемыйФайлНаДискеВХранилищеРасширениеПредложено

Асинх Процедура ПоместитьРедактируемыйФайлНаДискеВХранилищеРасширениеПредложеноЗавершение(Существует, ДополнительныеПараметры1) Экспорт
	
	ДополнительныеПараметры = ДополнительныеПараметры1.ДополнительныеПараметры;
	ИдентификаторФормы = ДополнительныеПараметры1.ИдентификаторФормы;
	ПолноеИмяФайлаНаКлиенте = ДополнительныеПараметры1.ПолноеИмяФайлаНаКлиенте;
	
	Если Существует Тогда
		ИнформацияОФайле = Ждать ПоместитьФайлВХранилище(ПолноеИмяФайлаНаКлиенте, ИдентификаторФормы);
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
КонецПроцедуры // ПоместитьРедактируемыйФайлНаДискеВХранилищеРасширениеПредложеноЗавершение
...
Процедура ВыбратьФайлНаДискеИПоместитьВХранилищеРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		ИнформацияОФайле = Неопределено;
		ВыборФайла.Показать(Новый ОписаниеОповещения("ВыбратьФайлНаДискеИПоместитьВХранилищеРасширениеПредложеноЗавершение", ЭтотОбъект, Новый Структура("ВыборФайла, ДополнительныеПараметры, ИдентификаторФормы", ВыборФайла, ДополнительныеПараметры, ИдентификаторФормы)));
	Иначе
...
Асинх Процедура ВыбратьФайлНаДискеИПоместитьВХранилищеРасширениеПредложеноЗавершение(ВыбранныеФайлы, ДополнительныеПараметры1) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		ИнформацияОФайле = Ждать ПоместитьФайлВХранилище(ВыборФайла.ПолноеИмяФайла, ИдентификаторФормы);
	КонецЕсли;
	
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикРезультата, ИнформацияОФайле);

КонецПроцедуры // ВыбратьФайлНаДискеИПоместитьВХранилищеРасширениеПредложеноЗавершение
...
Асинх Процедура ОткрытьКаталогСФайломРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ОткрытьКаталогСФайломРасширениеПредложеноЗавершение", ЭтотОбъект, Новый Структура("ДополнительныеПараметры, ПолныйПуть, РабочийКаталогПользователя, Файл", ДополнительныеПараметры, ПолныйПуть, РабочийКаталогПользователя, Файл)));
	Иначе
...
Процедура ОткрытьКаталогСФайломРасширениеПредложеноЗавершение(Существует, ДополнительныеПараметры1) Экспорт
	
	ДополнительныеПараметры = ДополнительныеПараметры1.ДополнительныеПараметры;
	ПолныйПуть = ДополнительныеПараметры1.ПолныйПуть;
	РабочийКаталогПользователя = ДополнительныеПараметры1.РабочийКаталогПользователя;
...
// Продолжение процедуры ОткрытьКаталогСФайлом.
Асинх Процедура ОткрытьКаталогСФайломОтветПолучен(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ПолноеИмяФайлаНаКлиенте = "";
	РезультатПолучения = Ждать ПолучитьФайлВРабочийКаталог(
...
Асинх Процедура ОповеститьОбИзмененииИУдалитьФайлВРабочемКаталогеРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ОповеститьОбИзмененииИУдалитьФайлВРабочемКаталогеРасширениеПредложеноЗавершение", ЭтотОбъект, Новый Структура("ПолныйПутьКФайлу, Файл", ПолныйПутьКФайлу, Файл)));
	КонецЕсли;
	
КонецПроцедуры

Асинх Процедура ОповеститьОбИзмененииИУдалитьФайлВРабочемКаталогеРасширениеПредложеноЗавершение(Существует, ДополнительныеПараметры1) Экспорт
	
	ПолныйПутьКФайлу 	= ДополнительныеПараметры1.ПолныйПутьКФайлу;
	Файл 				= ДополнительныеПараметры1.Файл;
	
	Если Существует Тогда
		Попытка
			Ждать Файл.УстановитьТолькоЧтениеАсинх(Ложь);
			Ждать УдалитьФайлыАсинх(ПолныйПутьКФайлу);
		Исключение
...
Асинх Процедура ПослеРасшифровкиФайла(ОписаниеДанных, ДополнительныеПараметры) Экспорт
	...
	#Иначе
		...
		Если НЕ ИзвлекатьТекстыФайловНаСервере Тогда
			...
			АдресВременногоХранилищаТекста = Ждать ИзвлечьТекстВоВременноеХранилище(
...
//  ИдентификаторФормы             - УникальныйИдентификатор формы.
//  ДопПараметры	   			   - Структура - необязательный параметр,
//									 позволяет передать дополнительные параметры, необходимые для записи 
//									 присоединенного файла (например, признак Чертеж).
//
Асинх Функция ПоместитьВыбранныеФайлыВХранилище(Знач ВыбранныеФайлы,
                                            Знач ВладелецФайла,
                                            ПрисоединенныеФайлыМассив,
	...
	// 22.03.2017 13:45:55 Савинов Александр 
	// Добавлен параметр ДопПараметры
	...
	Для Каждого ПолноеИмяФайла Из ВыбранныеФайлы Цикл
		...
		ВремяИзмененияУниверсальное = Ждать Файл.ПолучитьУниверсальноеВремяИзмененияАсинх();
		...
		РезультатПомещения = Ждать ПоместитьФайлыНаСерверАсинх(,,ПомещаемыеФайлы, ИдентификаторФормы);
		...
		АдресВременногоХранилищаФайла = РезультатПомещения[0].Адрес;
	...
	Возврат ПрисоединенныеФайлыМассив;
...
Асинх Процедура ОбновитьСостояниеОСохраненииФайлов(Знач ВыбранныеФайлы,
											 Знач Файл,
	...
	РазмерВМб = ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(Ждать Файл.РазмерАсинх() / (1024 * 1024));
...
Асинх Процедура ОткрытьФайлРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		ФайлНаДиске.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ОткрытьФайлРасширениеПредложеноЗавершение1", ЭтотОбъект, Новый Структура("ДанныеФайла, ДляРедактирования, ДополнительныеПараметры, ПолноеИмяФайлаНаКлиенте, ФайлНаДиске", ДанныеФайла, ДляРедактирования, ДополнительныеПараметры, ПолноеИмяФайлаНаКлиенте, ФайлНаДиске)));
...
Асинх Процедура ОткрытьФайлРасширениеПредложеноЗавершение1(Существует, ДополнительныеПараметры1) Экспорт
	
	ДанныеФайла = ДополнительныеПараметры1.ДанныеФайла;
	ДляРедактирования = ДополнительныеПараметры1.ДляРедактирования;
	ДополнительныеПараметры = ДополнительныеПараметры1.ДополнительныеПараметры;
	ПолноеИмяФайлаНаКлиенте = ДополнительныеПараметры1.ПолноеИмяФайлаНаКлиенте;
	ФайлНаДиске = ДополнительныеПараметры1.ФайлНаДиске;
	
	Если ЗначениеЗаполнено(ДанныеФайла.Редактирует) И ДляРедактирования И Существует Тогда
		ФайлНаДиске.НачатьУстановкуТолькоЧтения(Новый ОписаниеОповещения("ОткрытьФайлРасширениеПредложеноЗавершение", ЭтотОбъект, Новый Структура("ДанныеФайла, ДляРедактирования, ДополнительныеПараметры, ПолноеИмяФайлаНаКлиенте, ФайлНаДиске", ДанныеФайла, ДляРедактирования, ДополнительныеПараметры, ПолноеИмяФайлаНаКлиенте, ФайлНаДиске)), Ложь);
		Возврат;
	ИначеЕсли Ждать ФайлНаДиске.СуществуетАсинх() Тогда
	...
	ОткрытьФайлРасширениеПредложеноФрагмент(ДополнительныеПараметры, ПолучитьФайл);

КонецПроцедуры

Процедура ОткрытьФайлРасширениеПредложеноЗавершение(ДополнительныеПараметры1) Экспорт
...
Процедура ОткрытьФайлРасширениеПредложеноФрагмент(Знач ДополнительныеПараметры, Знач ПолучитьФайл)
...
// Продолжение процедуры ПрисоединенныеФайлыКлиент.ОткрытьФайл.
Асинх Процедура ОткрытьФайлДиалогПоказан(ПолучитьФайл, ДополнительныеПараметры) Экспорт
	...
	Если ПолучитьФайл Тогда
		ПолноеИмяФайлаНаКлиенте = "";
		РезультатПолучения = Ждать ПолучитьФайлВРабочийКаталог(
		//ФайлМожноОткрывать = Ждать ПолучитьФайлВРабочийКаталог(
			ДанныеФайла.СсылкаНаДвоичныеДанныеФайла,
			ДанныеФайла.ОтносительныйПуть,
			ДанныеФайла.ДатаМодификацииУниверсальная,
			ДанныеФайла.ИмяФайла,
			РабочийКаталогПользователя,
			ПолноеИмяФайлаНаКлиенте);
			Если ТипЗнч(РезультатПолучения) = Тип("Структура") Тогда
				ФайлМожноОткрывать 		= РезультатПолучения.ФайлМожноОткрывать;
				ПолноеИмяФайлаНаКлиенте = РезультатПолучения.ПолноеИмяФайлаНаКлиенте;
			КонецЕсли;
	КонецЕсли;
		
	Если ФайлМожноОткрывать Тогда
		Если ДляРедактирования Тогда
			Ждать ФайлНаДиске.УстановитьТолькоЧтениеАсинх(Ложь);
		Иначе
			Ждать ФайлНаДиске.УстановитьТолькоЧтениеАсинх(Истина);
		КонецЕсли;
...
//
Асинх Процедура ПриСохраненииДанныхФайла(Параметры, ПараметрыВыполнения) Экспорт
	
	РасширениеПодключено = Ждать ФайловыеФункцииСлужебныйКлиент.РасширениеРаботыСФайламиПодключено();
...
Процедура СохранитьФайлКакРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		ВыборФайла.Показать(Новый ОписаниеОповещения("СохранитьФайлКакРасширениеПредложеноЗавершение", ЭтотОбъект, Новый Структура("ВыборФайла, ДанныеФайла, ДополнительныеПараметры, ПолноеИмяФайла", ВыборФайла, ДанныеФайла, ДополнительныеПараметры, ПолноеИмяФайла)));
        Возврат;
	Иначе
	...
	СохранитьФайлКакРасширениеПредложеноФрагмент1(ДополнительныеПараметры, ПолноеИмяФайла);
КонецПроцедуры

Процедура СохранитьФайлКакРасширениеПредложеноЗавершение(ВыбранныеФайлы, ДополнительныеПараметры1) Экспорт
	...
	Если НЕ (ВыбранныеФайлы <> Неопределено) Тогда
		Возврат;
	КонецЕсли; // Если НЕ (ВыбранныеФайлы <> Неопределено) Тогда
	...
	ДопПараметры = Новый Структура("ВыборФайла", ВыборФайла);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеПолученияФайловСохранитьКакРасширение", ПрисоединенныеФайлыСлужебныйКлиент, ДопПараметры);
	
	НачатьПолучениеФайлов(ОписаниеОповещения,,,Ложь); 
...
Процедура ПослеПолученияФайловСохранитьКакРасширение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
...
КонецПроцедуры // ПослеПолученияФайловСохранитьКакРасширение

Процедура СохранитьФайлКакРасширениеПредложеноФрагмент1(Знач ДополнительныеПараметры, Знач ПолноеИмяФайла)
	
	СохранитьФайлКакРасширениеПредложеноФрагмент(ДополнительныеПараметры, ПолноеИмяФайла);

КонецПроцедуры

Процедура СохранитьФайлКакРасширениеПредложеноФрагмент(Знач ДополнительныеПараметры, Знач ПолноеИмяФайла)
...
Процедура ПоказатьДиалогНужноПолучитьФайлССервераДействиеОпределено(Действие, ДополнительныеПараметры) Экспорт
	...
	Если Действие = "ВзятьИзХранилищаИОткрыть" Тогда
		...
		Файл.НачатьУстановкуТолькоЧтения(Новый ОписаниеОповещения("ПоказатьДиалогНужноПолучитьФайлССервераДействиеОпределеноЗавершение1", ЭтотОбъект, Новый Структура("Действие, ДополнительныеПараметры, ИмяФайлаСПутем", Действие, ДополнительныеПараметры, ИмяФайлаСПутем)), Ложь);
        Возврат;
	...
	ПоказатьДиалогНужноПолучитьФайлССервераДействиеОпределеноФрагмент(ДополнительныеПараметры, Результат);
КонецПроцедуры

Процедура ПоказатьДиалогНужноПолучитьФайлССервераДействиеОпределеноЗавершение1(ДополнительныеПараметры1) Экспорт
	
	Действие = ДополнительныеПараметры1.Действие;
	ДополнительныеПараметры = ДополнительныеПараметры1.ДополнительныеПараметры;
	ИмяФайлаСПутем = ДополнительныеПараметры1.ИмяФайлаСПутем;
	
	НачатьУдалениеФайлов(Новый ОписаниеОповещения("ПоказатьДиалогНужноПолучитьФайлССервераДействиеОпределеноЗавершение", ЭтотОбъект, Новый Структура("Действие, ДополнительныеПараметры", Действие, ДополнительныеПараметры)), ИмяФайлаСПутем);

КонецПроцедуры

Процедура ПоказатьДиалогНужноПолучитьФайлССервераДействиеОпределеноЗавершение(ДополнительныеПараметры1) Экспорт
	Действие = ДополнительныеПараметры1.Действие;
	ДополнительныеПараметры = ДополнительныеПараметры1.ДополнительныеПараметры;
	Результат = Истина;
	ПоказатьДиалогНужноПолучитьФайлССервераДействиеОпределеноФрагмент(ДополнительныеПараметры, Результат);
...
Процедура ПоказатьДиалогНужноПолучитьФайлССервераДействиеОпределеноФрагмент(Знач ДополнительныеПараметры, Знач Результат)
	
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикРезультата, Результат);

КонецПроцедуры

Асинх Процедура ОткрытьФайлПриложением(Знач ИмяОткрываемогоФайла, ДанныеФайла)
	...
	Если РасширениеПодключено Тогда
		...
		Если НРег(ДанныеФайла.Расширение) = НРег("mxl") Тогда
			...
			ПомещенныеФайлы = Новый Массив;
			НачатьПомещениеФайлов(Новый ОписаниеОповещения("ОткрытьФайлПриложениемЗавершение", ЭтотОбъект, Новый Структура("ЗаголовокСтрока, ИмяОткрываемогоФайла, ПомещенныеФайлы", ЗаголовокСтрока, ИмяОткрываемогоФайла, ПомещенныеФайлы)), ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь);
			Возврат;
		КонецЕсли;
		
		// Открытие Файла.
		Попытка
			Ждать ЗапуститьПриложениеАсинх(ИмяОткрываемогоФайла);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ОткрытьФайлПриложениемЗавершение(ПомещенныеФайлы1, ДополнительныеПараметры) Экспорт
	
	ЗаголовокСтрока = ДополнительныеПараметры.ЗаголовокСтрока;
...
КонецПроцедуры // ОткрытьФайлПриложениемЗавершение
